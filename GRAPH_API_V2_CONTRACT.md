...
> **상태(2026-07-21)**: **서버 slice(7f4e69a) + iOS one-shot consumer(c31dfe9→fb9d15c) 모두 구현**. 서버는 refresh_not_before 부착(additive), iOS는 5분 폴링+20초×9 재시도를 단일 scheduler로 교체(load-owns-reschedule / schedulerFetchingPeriod 소유권 / due-check nextEligibleAt 존중 / cancellation 분리 / 선택탭 게이트 / preload per-period 실패 cooldown, codex 7라운드 하드닝[최종 코드 finding 0], FXiTests 342 passed). **서버 배포 완료(2026-07-21, HEAD 21398e3 / image bd111ee)** — 라이브 canonical에 endpoint와 동일한 attach를 직접 적용한 **응답 본문 조립 검증**(실제 HTTP 200 아님) 통과: top-level `refresh_not_before` = 다음 :31:00 +09:00, canonical immutable(copy 부착), usd 4 period 전수, serve/cron 동일 REDIS_URL DB 0. **실제 인증 HTTP 200 수신 + one-shot(:31+jitter 발화)/탭왕복 무추가 GET 실측은 iOS dev build E2E에서 최종 close** — additive라 구 프로덕션 앱 무영향.
