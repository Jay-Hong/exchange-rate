...
| 항목 | 값 |
| --- | --- |
| 배포된 코드 | `721a3237900961cf59e9a50a98ce20357bc56755` |
| 실행 이미지 = `latest` | `sha256:eec4a8025dcb99531598462d6e8cbcf19a70bdb7e99ccff87f8cd90e637b9b03` |
| 코드 롤백 앵커 | `exchange-rate-fastapi:rollback-1789661986306132298` (= 직전 이미지 `f6738338e173…a3b`) |
| IBK 설정 | `IBK_RESULT_PATH_ENABLED=true`, `TELEGRAM_ENABLED=true` (**둘 다 코드 기본값은 `false`**) |
| 현재 Compose 라벨 | `/home/ubuntu/fxi-release-721a323-20260918-01/new.json` |
| 동결 모델 SHA-256 | `5de7d922086e82c7502dcd2c665e576215fad88ebead5b743e5f437e9039fc82` |
| 같은 실행의 구 모델 SHA-256 | `6ee6c0a4e0b4c7cc2ca728f70c47fd60002041166d75bafc891090e27a99422a` (= 직전 배포의 `new.json`) |
| 배포 도구 | `/home/ubuntu/fxi-release-721a323-v5.py` SHA-256 `12f17dbf…dcbca` (**전환 1회용**) |

⚠️ **롤백 앵커 태그 이름을 세대와 혼동하지 말 것.** 이번 배포 시점에 `rollback-1789082062562934420`
...
이 명령은 **2026-09-10 활성화 실행의 복구 전용**이며, 그 뒤로 코드가 두 번(`d80ea73`,
`721a323`) 바뀌었으므로 **그대로 실행하지 않는다.** 당시 도구의 사전 조건과 현재 상태를
대조한 뒤에만 쓴다. 되돌리면 legacy의 MIBANK 저장 정책도 함께 복원되고, **이미 저장된 DB
데이터는 되돌아가지 않는다.**

⛔ **다음 배포가 알아야 할 것 셋.**
(1) 현재 컨테이너는 canonical `docker-compose.yml` 이 아니라 위 표의 동결 모델
(`fxi-release-721a323-20260918-01/new.json`)로 만들어져 있다. 배포 도구가 그 상태를
