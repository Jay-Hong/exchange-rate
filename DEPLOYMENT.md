...
### 8.1b 현재 운영 기준선 (2026-09-18 — 배포 직전 서버와 반드시 대조)

⛔ 이 표는 **참고**다. 권위는 서버에 있고, 다음 배포는 **서버 실측값에서 출발**한다.

| 항목 | 값 |
| --- | --- |
| 배포된 코드 | `13bc1a31f16200ff0400c0b1e12b8514dbee7853` |
| 실행 이미지 = `latest` | `sha256:683de70ae4d24ba222eee9f058b0b920a130c80ff4909381ff6e868cc36d234c` |
| 코드 롤백 앵커 | `exchange-rate-fastapi:rollback-1789704768483983729` (= 직전 이미지 `eec4a8025dcb…9b03`) |
| IBK 설정 | `IBK_RESULT_PATH_ENABLED=true`, `TELEGRAM_ENABLED=true` (**둘 다 코드 기본값은 `false`**) |
| 현재 Compose 라벨 | `/home/ubuntu/fxi-release-13bc1a3-20260918-01/new.json` |
| 동결 모델 SHA-256 | `a3184d2973c9426440d216c6da0f21a0b53331700275dd547029d6f9d6774204` |
| 같은 실행의 구 모델 SHA-256 | `5de7d922086e82c7502dcd2c665e576215fad88ebead5b743e5f437e9039fc82` (= 직전 배포의 `new.json`) |
| 배포 도구 | `/home/ubuntu/fxi-release-13bc1a3-v6.py` SHA-256 `b523ac1f…f08b` (**전환 1회용**) |
...
이 명령은 **2026-09-10 활성화 실행의 복구 전용**이며, 그 뒤로 코드가 세 번(`d80ea73`,
`721a323`, `13bc1a3`) 바뀌었으므로 **그대로 실행하지 않는다.** 당시 도구의 사전 조건과 현재 상태를
대조한 뒤에만 쓴다. 되돌리면 legacy의 MIBANK 저장 정책도 함께 복원되고, **이미 저장된 DB
데이터는 되돌아가지 않는다.**

⛔ **다음 배포가 알아야 할 것 셋.**
(1) 현재 컨테이너는 canonical `docker-compose.yml` 이 아니라 위 표의 동결 모델
(`fxi-release-13bc1a3-20260918-01/new.json`)로 만들어져 있다. 배포 도구가 그 상태를
...
`13bc1a3` 배포(v6)에서는 직전 기록(v5 가 쓴 `state.json`·`switch-verify.json`·`operator-promoted.json`)이
v5 가 검증하던 필드를 모두 가진 **같은 스키마**여서 상수만 바꿨다 — 다음에도 먼저 서버에서 대조한다.
⚠️ v6 에는 낡은 주석 두 곳("구 이미지에 추출기가 없다", "cron 5건")이 남아 있다. 실행 논리와 무관하지만
근거로 쓰지 않는다.

⚠️ **열린 정정 항목**: `CLAUDE.md` 의 "현재 운영 기준 (2026-09-10) 코드 `9d4e1c4`" 는 `d80ea73` 배포 이후
낡았다. `CLAUDE.md` 는 C2 인용 경로라 정정은 C2 재결속과 함께 별도 커밋으로 한다.
