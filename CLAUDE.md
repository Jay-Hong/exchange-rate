...
- 🟡 REST snapshot/fallback ([ADR-027](DECISIONS.md#adr-027-krx-미국달러선물-stage-2-진입-전-rest-snapshotfallback--stale-정책-초안)) — WebSocket 끊김 시 최신성 보전. **telemetry canary 활성화 (2026-06-07 주말)**: guard(판정+telemetry) + robust calendar 배포(`23df2d2`) + `KRX_REST_FALLBACK_ENABLED=true` + recreate (verified: health·calendar live·KRX bootstrap A75606 ERROR 0). **write/broadcast 0 (telemetry-only)** → 주말 idle, **월요일 6/8 개장부터 guard 발화 + 첫 telemetry(`rest_guard_*`·`rest_success/error`) 관찰 예정** (결과는 추후 보강). 4-gate(calendar + session + active contract + freshness → fill/exclude). **Stage C 실 반영(topic/Redis write)은 미구현** — ADR-028 topic + KRX Stage 2 선행. 5/25 교훈: close snapshot 차단(`KRX_CLOSE_REST_WRITE_ENABLED=false`) / general fallback 장중 + guard. PR6c-2d-1 자동 rollover로 만기 layer 분리.
