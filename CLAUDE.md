...
- ⏸ REST snapshot/fallback ([ADR-027](DECISIONS.md#adr-027-krx-미국달러선물-stage-2-진입-전-rest-snapshotfallback--stale-정책-초안)) — WebSocket 끊김 시 최신성 보전. **Stage A/B 구현·배포 완료** (`KRX_REST_FALLBACK_ENABLED=false`, Stage B=log/counter-only — 반영 경로 0). **Stage C(반영)는 freshness guard 선행 필요** — 2026-06-07 **4-gate 설계** land (calendar + session + active contract + freshness → fill/exclude, ADR-027 §Stage C guard 설계). 5/25 휴장일 stale REST 사고 교훈: close snapshot은 차단(`KRX_CLOSE_REST_WRITE_ENABLED=false`), general fallback은 장중 한정 + guard 통과 시만 fill / 실패 시 exclude·telemetry. PR6c-2d-1 자동 rollover로 만기 layer 분리.
