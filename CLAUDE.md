...
> 🧩 **All-source fanout scope-lock:** [ALL_SOURCE_FANOUT_UNIFICATION_PLAN.md](ALL_SOURCE_FANOUT_UNIFICATION_PLAN.md) - 3 path(Bank/Investing · USDT 5 · KRX) 통합 **scope-lock (plan-only)**: 공통화 대상 / 절대 통합 금지(never-unify 9항목) / open decision / staged step. 근본 불변식 = FX multi-process→atomic vs USDT/KRX single-process→in-memory 가드. 계약 본문은 REALTIME §4.1 재사용.
...
> 📡 **Realtime V2 클라이언트 계약:** [REALTIME_V2_CLIENT_GUIDE.md](REALTIME_V2_CLIENT_GUIDE.md) - **신규 topic-consuming 앱 핸드오프 단일 계약 (2026-06-25 신설, Draft)**. 지원 topic(fx:\*/usdt:krw/krx:usd-krw-futures[G2∧G3 on일 때만]) · subscribe/snapshot-on-subscribe · payload schema · (source,asset) timestamp-merge race 계약 · REST bootstrap(`GET /api/v2/topics/snapshot`) · versioning. **구 OPEN 2건(usdt:krw REST bootstrap / USDT same-bucket ordering)은 모두 해소**. 서버 코드 구현 완료·**prod flag OFF**(2026-07-22 route auth 감사 완화). REST bootstrap은 2026-07-25부터 **인증+premium+per-user KRX 강제**(ADR-039 §8.1 E3). 재활성화 선행 = E3 + WS 인증(1C) + 클라 bootstrap 인증 이관, live 활성 별도 GO
...
- **BREAK1 모드**: 월~금 19:00 ~ 익일 06:00 (야간, 10개 크롤러)
  ...
  - **은행별 종료 시각**(cron 시간 범위로 표현, 별도 런타임 게이트 없음):
    ...
    - woori `hour='19-23,0-5'` `:14/:44` → 마지막 **05:59:44**(1,320회). 이어서 BREAK2의
      마무리 조회가 `06:00~06:03 :14/:44` + `06:04:53`(화~토, 9회)를 맡는다 —
      **같은 `task_woori` ID**여야 전환을 걸친 실행과 배타가 성립한다
...
- **BREAK2 모드**: 06:00~07:59 (고시 마무리, 7개 크롤러)
  - 제외: shinhan(03:00 종료), sc(19:00 종료). woori는 정규 job 없이 마무리 조회만
...
| **woori** | 평일 08:30 | **05:55:56 관측(2026-09-05), 종료 시각 미확정** | 24시간 외환시장 전환으로 연장. 06:00은 관측이 아니라 우리가 정한 **수집 종료 정책**. 야간 수집 ~05:59:44 + 마무리 조회 06:04:53(화~토) |
...
- **BREAK1**: kb·hana는 IN과 같은 10초 레인, bs·citi 유지. woori는 19:00~**05:59**에
  `:14/:44`로 돌고 마지막 발화는 **05:59:44**다(총 1,320회). 이어서 BREAK2의 마무리 조회가
  `06:00~06:03 :14/:44` + `06:04:53`(화~토, 9회)를 맡는다.
  ⛔ 마무리 조회는 **같은 `task_woori` ID**여야 한다 — APScheduler executor의
  `_instances[job.id]`는 remove/add로 사라지지 않아, 같은 ID면 05:59:44 실행이 06:00 전환을
  걸쳐 살아 있을 때 마무리 발화가 차단된다. 다른 ID로 나누면 별도 카운터라 Chrome 2개가
  동시에 뜬다. **시간 간격으로는 보장되지 않는다.**
- **BREAK2**: kb·hana는 IN과 같은 10초 레인, bs·citi 유지. woori는 마무리 조회만(화~토)
...
- **BREAK2**: nh(54초) 유지 + `task_ibk_terminal`(06:00:34·06:01:34, 화~토).
  shinhan(03:00 종료) / sc(19:00 종료) 제외. woori는 마무리 조회 `06:00~06:04:53`(화~토)
...
- kb·hana는 IN/BREAK1/BREAK2에서 10초, woori는 IN/BREAK1에서 30초로 수집한다.
  OUT 주기와 woori의 OUT 제외는 유지한다 (woori BREAK2 마무리 조회는 ADR-044)
...
**현재 운영 기준선**(배포 코드·이미지·설정값·동결 Compose 모델·복구 기준)은
[DEPLOYMENT.md](DEPLOYMENT.md) §8.1b 에 기록한다. 이 표는 참고 기준이며, 배포 직전 실제 서버
상태와 반드시 대조한다. ⛔ 여기에 배포 SHA·설정값을 다시 적지 않는다 — 배포마다 낡는다.
...
- ✅ **만기일 휴장 보정 (2026-08-17)**: 만기는 셋째 월요일이지만 **그날이 휴장이면 직전 영업일로 앞당겨진다**. 2026-08-17(월) 광복절 대체공휴일 → 8월물 만기는 **2026-08-14(금)**. 단일 진실 소스 `app/calendars/krx_calendar.py`(`usdf_expiry_date` / `is_krx_regular_business_day` / `is_krx_night_session_open`, `holidays.SouthKorea(PUBLIC∪BANK, observed=True)` 기반이라 연도 하드코딩 없음). `get_active_session(now, contract_expiry_date)`는 **필수 인자**(구 `None` legacy 호환 폐기 — 빠뜨리면 조용히 기본값으로 돌지 않고 TypeError). `is_expiry_day`·`KRX_2026_USDF_EXPIRY_DAYS` 삭제, scheduler 점프 보호는 `expiry_diff > 45일` → `_is_next_contract_month` 월물 비교. `2026-02-13` 야간 휴장은 설 연휴 전 **일회성**(KIND acptno=20260206002546)이라 override 표에 근거와 함께 고정 — 연례 규칙 아님. **데이터 영향**: segment 경계 이동으로 교정 전 적재분 주중 2일자가 어긋남(`2026-02-13` contract 오귀속 / `2026-08-14` 행 부재) → `scripts/repair_krx_daily_rows.py` allowlist 교정(별도 GO). 상세: [DECISIONS.md ADR-027 Amendment](DECISIONS.md) + [GRAPH_API_V2_CONTRACT.md §7-new](GRAPH_API_V2_CONTRACT.md) + 배포 runbook은 [KRX_CANARY.md](KRX_CANARY.md)
...
- ✅ **PR D / P1b A1 land — atomic-write control plane foundation (dormant, behavior-change-0)** (2026-06-17, [P1_COMMON_BASE_DESIGN.md §19](P1_COMMON_BASE_DESIGN.md) A1): `AtomicWriteControl` 싱글톤 control table([§4](P1_COMMON_BASE_DESIGN.md) — **리포 첫 DB CHECK 3개**[id=1/requested_mode enum/activation_epoch≥0], ADR-034 'no CHECK' 선례와 의도적 divergence[control-plane 무결성], `activated_at` nullable[activation 전 None]) + `app/atomic_write_control.py`(`compute_effective_mode` §3/§7 fail-closed — **dormant: 어떤 writer도 미호출**, AST trip-wire 테스트로 경계 잠금) + `app/database.create_all_app_tables`(control table을 create_all에서 제외 → `scripts/migrate_atomic_write_control.py`가 **운영(non-test) 유일 생성 경로**, import-time CHECK DDL 위험 0; main.py + backfill_history.py 공유, 테스트는 control table 필요해 직접 create) + read-only never-crash status endpoint(`/admin/api/atomic-write-control-status`, `writer_enforced=false`). **codex CLI 4-round 리뷰(최종 High 게이트 포함) 수렴**(create_all 누락[Major B]·schema 가드[Major A]·activated_at must-fix 등 독립 검증 후 반영, #6/mode_generation CHECK는 검증 후 기각). 23 tests + 전체 suite green. writer enforcement는 A2(dormancy 해제). 분해: A1→A2→A3→A4 → {A5, B1→B2a→B2b→B3} → C6 → C7.
- ✅ **PR D / P1b C6 atomic-FX cutover EXECUTED & COMPLETE (production)** (2026-06-21, [P1_COMMON_BASE_DESIGN.md §19](P1_COMMON_BASE_DESIGN.md) "🎯 현재 상태"): A→B→C6 dormant series 전부 land(A1~A5/B1~B3/C6-1~9/C6-5b/C6-quiesce Q0~Q4b/W0~W3/RELEASE, behavior-change-0) 후 production flip 실행. **scope = FX(bank+investing) usd/jpy/eur only** (writer mode는 전역). **타임라인**: DEPLOY+G2a(`c210e7f`) → **EXEC-1~5 첫 시도 성공·EXEC-6 실패**(startup-ordering incident: app startup이 `refresh_write_mode_cache` 전에 `warmup_latest_rates` 실행 → `_INITIAL`=legacy라 warmup이 EXEC-3의 v2 Redis를 v1으로 덮음 → atomic mirror `migration_required×30` 영구 stuck) → **ROLLBACK A**(pre-C6 image `4c8536d` 재배포 → control 무시·legacy v1 write → 서비스 완전 복원; "코드 비가역 flip ≠ 서비스 복구 불가") → **startup-ordering FIX `a756d31`**(refresh→warmup 순서 + `is_initialized()` 가드: write-mode 미확정 시 FX writer/mirror legacy write 금지; codex 3 NO-GO fix) → orphaned control reset + fixed image 배포 → **EXEC-1~8 재시도 전 단계 성공 + post-EXEC-6 5-signal HARD GATE 5/5 PASS**(~21:08 KST, incident 재발 0) = **cutover_state=atomic_ready / publisher_gate_open=true / writer atomic epoch1 / 3 asset ready**. **교훈(no-staging)**: dormant 코드 + CI green이어도 "prod에서 그 경로가 처음 켜질 때"의 startup race는 CI(sqlite/단발 import)가 못 잡음 → mode 전환은 startup 시퀀스 재현 test 필요 + rollback 경로(old image) 사전 식별. **잔존 = C7(post-flip cleanup)**. 상세: memory [[feedback_atomic_cutover_incident]] + [[project_c6_plan_entry]].
- ✅ **P1b USDT/KRX Redis writer decouple hotfix** (2026-06-22, `de4fe41`, [P1 §401-405 Amendment](P1_COMMON_BASE_DESIGN.md)): C6 cutover의 **전역 writer mode가 USDT/KRX source Redis writer까지 BLOCK**해 iOS 테스트 테더 탭 freeze(SET-only topic trigger → BLOCKED → no push; DB는 mode-independent라 데이터 안전). **USDT/KRX `latest:source:*`는 FX atomic keyspace와 disjoint**(atomic loader는 bank/investing만 read, mirror는 source allowlist-skip[source_skipped=0])라 §11 FX v2 invariant 밖 → DB writer처럼 Redis writer도 **mode-independent로 정렬**. **4 gate 제거**(usdt setter / krx base 598 / krx tick 675 / `krx_kis._sync_db_write` flag=false 분기 — 4번째는 `_record_write_mode_block` 미경유라 design Workflow가 놓쳤으나 검증 모드 재검증이 포착). KEEP dormant scaffolding(BLOCKED enum + helper, per-source self-cutover gate용). guardrail: 3 setter + `_sync_db_write` 전수 mode-independent test + AST trip-wire. **전제: 단일 프로세스(`--workers 1`)** — in-memory coalesce/regression 가드 load-bearing, multi-process 시 재평가. 배포·검증 완료(FX atomic 무영향 + USDT/테더 탭 복구, KRX는 월요일 세션부터). **USDT/KRX full atomic = 열린 결정**(atomic이 푸는 cross-process race가 단일프로세스 USDT/KRX엔 없음 — topology 변화 시 재평가). KRX in-memory 역행가드(cross-schema cold-start 주의)는 별도 PR. 상세: memory [[project_atomic_scope_usdt_krx]].
- 🔜 **All-source observation fanout 통합 phase** — [REALTIME_ARCHITECTURE_PLAN.md §4.1](REALTIME_ARCHITECTURE_PLAN.md) anchor. **✅ scope-lock land (2026-06-23, plan-only): [ALL_SOURCE_FANOUT_UNIFICATION_PLAN.md](ALL_SOURCE_FANOUT_UNIFICATION_PLAN.md)** — 3 path 코드 매핑(Workflow + codex 검토 7 이슈 반영) → 공통화 대상 / never-unify 9항목 / open decision 7 / staged step 8. 구현은 별도 세션(작은 슬라이스). 남은 순서: (1) [Bank/Investing β](USDT_TOPIC_MIGRATION_PLAN.md) (DB-first monolithic → observation fanout 재설계 + main.py legacy hook 격하 가능성 — 단 FX C1 trigger는 이미 direct 활성, backward-flip 금지) → (2) USDT 5 source 공통화 검토 (3 도메인 검증 패턴 input, 선제 abstraction 금지 원칙). **mirror retirement (후속, P1 범위 밖)**: ADR-026 3s latest mirror는 broadcast-off-DB 1세대 해법 — P1/PR D는 mirror 유지(direct+mirror revision-aware atomic화)이고 제거는 본 fanout 트랙. 3s polling은 freshness/DB-fallback 회피 역할 겸해 **naive 제거 금지** → measure first → freshness 의미 분리 → targeted reconciliation/audit → cadence 축소·제거, **replace-before-remove**. 주목적 = polling 의존 제거(DB 부하 절감은 재측정 후 판단). 상세 [REALTIME_ARCHITECTURE_PLAN.md §4.1.5](REALTIME_ARCHITECTURE_PLAN.md).
