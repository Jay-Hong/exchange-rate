...
**핵심 가이드:**
> 💡 **아키텍처 의사결정:** [DECISIONS.md](DECISIONS.md) - 주요 기술 선택과 그 근거 (ADR). **최신: ADR-031 (KRX Redis 1차 통합), ADR-032 (KRX 가격알림 evaluator — F-1/F-2/F-3 trilogy), ADR-033 (Graph API v2 catalog policy + Amendment 2026-05-27 — Hana 자체 historical 단일 source / Bithumb·KRX external_historical / daily rollup 우선순위 ↓ (장기 coverage 목적) / DXY_futures 1d only / Citi v2 catalog 제외)**
> 📊 **Graph API v2 design:** [GRAPH_API_V2_CONTRACT.md](GRAPH_API_V2_CONTRACT.md) - catalog matrix (tab × period × series), provenance schema, **Hana official historical source rule + KRX contract chain rollover boundary policy** (Amendment 2026-05-27), external_historical + insufficient_history fallback, endpoint contract
...
- ✅ **Phase 2c external_historical source 확정 + ADR-033 Amendment 2026-05-27** (2026-05-27): Bithumb 공식 candlestick API (902일, 무인증, ccxt OCHL schema 잠금) / KRX KIS `inquire-daily-fuopchartprice` + A75YMM contract chain (만기 지난 월물 조회 가능, monthly front-month chain이면 1y는 최대 12 contracts, 각 contract 사용 구간만 좁게 fetch하면 보통 100건 cap 회피) / Hana official historical endpoint (20년+, 휴일 자동 fallback, 매매기준율 인덱스 7, 회차 정책 Phase 2e 전 확정). ADR-033 Decision 5 (Hana = Investing backfill → Hana 자체 historical 단일 source) + Decision 7 (Bithumb/KRX = insufficient_history → external_historical 확정, insufficient_history는 신규 자산/source 장애에 여전 유효) + Decision 10 (장기 coverage 목적 daily rollup 우선순위 ↓, internal cache/장애 대비 목적은 future optimization 보존) Amendment. GRAPH_API_V2_CONTRACT.md §7 Hana backfill merge rule은 §7 Hana official historical source rule + §7-new KRX contract chain rollover boundary policy로 대체. 상세: [DECISIONS.md ADR-033 Amendment](DECISIONS.md) + [GRAPH_API_V2_CONTRACT.md](GRAPH_API_V2_CONTRACT.md).
