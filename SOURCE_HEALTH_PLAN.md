...
- **D1 직함 문맥 탐지**(2026-09-21 추가, 5a 부터 **캡처 도구가 부른다**, 5b 에 반입 판정 라이브러리 — **실제 fixture 반입 시험 연결은 5c**): 명세는 리포 밖
  ...
  **슬라이스 5b(반입 판정 라이브러리)**: 5b-1 에서 `query_key` 를 순수 모듈 `queries.py` 로 떼어 detector 가 등록부·앱 import 경계에 닿지 않게
  ...
  manifest 가 **정확히 같은지** 시험한다. ⛔ 아직 실제 fixture 에 이 판정을 돌리는 시험은 없다 — C1d 의 실제 파일 시험 재기준화와
  씨티 2개·부산 공식 재캡처·반입은 5c(사용자 승인).
