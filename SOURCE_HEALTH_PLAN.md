...
> **D7 설계 합의(2026-09-22 Claude·Codex)** → [SOURCE_HEALTH_D7_AGGREGATION.md](SOURCE_HEALTH_D7_AGGREGATION.md). log-only shadow 집계 계약이다 —
> 구 `crawler_stats` 동결(쿨다운 이행 예외 포함), "판정된 회차 중" 수집률 + 판정 가능 비율, 래퍼 진입 등록 기반 보고 완결성 cohort, 겹치지 않는 1분 확정 버킷,
> epoch 식별 보존과 포화 시 접수 중단(약 11일 무재시작이 운영 한계). 구현·API/화면 노출은 별도 결정이다.
...
- **한계**: commit 뒤 `driver.quit()` 멈춤 → 프레임 없음 → `unknown`. **자식의** 보고 I/O(직렬화·쓰기·이름 바꾸기)가 45초 예산 안이라
  ...
  판정하지 않는다(§7.7).
- **R1 관측 뒤 확정**: 실측 크기·로그 증가율·상한·압축, 실제 예외 분포, `bank_report` 공통화 범위 → 아래 보완에서 정했다
  (공통화 = 자식이 기존 `bank_report` 관측자를 재사용). 실제 예외 분포는 E0 관측으로 남는다.

#### §7.13 보완 — 크기·로그량·프레임 검증·결속 (2026-09-22 Claude·Codex 설계 합의)

⛔ 설계만. 위 합의를 바꾸지 않고 채운다. 구현 확정은 여전히 E0 관측·프레임 격리 시험 뒤.

##### 1. 정책상 예정 실행 기회 수 (분 단위 순회, 2026-09-28 주 — Codex AST 재현 일치)

| 요일 | hana | woori | 합 |
|---|---:|---:|---:|
| 월 | 6,840 | 1,920 | 8,760 |
| 화~금(각) | 8,640 | 2,649 | 11,289 |
| 토 | 3,540 | 729 | 4,269 |
| 일 | 1,440 | 0 | 1,440 |
| 주 | 46,380 | 13,245 | 59,625 |

**실제 완료 회차가 아니다** — 관리자 비활성·재시작·misfire·max_instances(우리 마무리 9회도 앞 실행이 남으면 건너뜀)로 줄어든다.

##### 2. 부모 종료 보고 크기와 로그량
R1(1,797회차, 중복 0) 재측정 — message / 앱 JSON(Docker 접두 시각·마지막 개행 제외):

| | message 중앙/최대 | 앱 JSON 중앙/최대 |
|---|---:|---:|
| citi(전부 공식 1차 성공) | 5,072 / 5,074 | 5,954 / 5,956 |
| bs 공식 1차 성공 896회 | — / 4,973 | — / 6,092 |
| bs 전체(최대 = 공식 실패 → MIBANK 성공 회차) | 4,972 / 6,161 | 6,091 / 7,532 |

- 5,000/6,000 B 가정 → 화~금 하루 **56.445 / 67.734 MB**, 주 **298.125 / 357.750 MB**. ⚠️ **종료 이벤트만**의 추정이다.
  `bank_round_started`, 폴백 관련 로그, 폴백 회차의 더 큰 종료 보고는 빠져 있다.
- 보존창 영향은 **숫자로 주장하지 않는다**: 현재 유입(~192 MB/day)은 json-file 계층, 위 증가량은 앱 JSON 계층이라 비율을 낼 수 없다.
  배포 후 **같은 계층(json-file)** 증가율로 재계산한다.
- **선행 조건**: 로그 보관기의 완료 조건(LOG_RETENTION.md 설치 4단계) 충족 — 연속 정기 실행 종료 0·누락 없음, 양쪽 스트림 복원(해시) 검증,
  **첫 24시간 실측 크기** — 뒤에만 hana·woori 보고를 배포한다.
- **압축형은 이 슬라이스의 선행 작업이 아니다.** R1 의 통화별 수집 판정 5,391건 전부 `unknown/v2_evidence_unconfirmed` 라 "전 통화 valid" 조건의
  압축 대상이 없다. investing 의 76.8% 는 은행에 옮길 수 없다. 뒤에 압축하더라도 은행의 통화별 writer 증거를 정수 반환으로 대체하지 않는다.

##### 3. 자식 프레임 내용·크기
- 자식은 §7.13 대로 **기존 `bank_report` 관측자를 재사용**한다 → 기존 상한이 그대로 적용된다: 근거 조각 `SNIPPET_MAX_CHARS=160`·`SNIPPET_MAX_BYTES=640`
  (bank_report.py:38-39, 잘림 표시 + 공백 정규화 뒤·자르기 전 길이 `original_length`, :57-66), 통화별 관측 8·miss 8(:40·42), 필수 밖 코드 64×16자(:44-45).
- **상한은 보고용 사본에만.** 원문 파싱·writer 입력·폴백은 원문 그대로. 관측자는 잘라도 `finite`·`rate`(비유한은 문자열) 를 원문 파싱 결과로 보존하고
  (bank_report.py:367-374), 부모 판정은 확인된 위반(`non_finite`·충돌) 을 먼저 낸다(:575-590). 이 보존은 **텍스트 조각 잘림**에 관한 것이다 — 목록 상한으로 아예 버려진 관측은 보존되지 않는다. ⛔ 텍스트가 잘렸다는 이유로 관측을 `unknown` 으로
  덮지 않는다 — 잘림 때문에 판정할 수 없어진 부분(예: 충돌·완결 확정)만 unknown.
- 기존 관측·miss 상한은 **경로(attempt) 단위**다(`attempt["observations"]`). woori 자식은 한 경로 안에서 최대 11조회
  (오늘 + `MAX_DAYS_LOOKBACK=10`, constants.py:100)를 돈다 → 상한 단위는 §6-1 로 정했다.
- **크기 여유는 아직 주장하지 않는다.** 합성 측정(Codex): 256 코드포인트 × 33 문자열만으로 비BMP·`ensure_ascii=False` 33,892 B / 제어문자 50,788 B /
  비BMP·`ensure_ascii=True` 101,476 B. 실제 schema·직렬화 설정(ensure_ascii·구분자)을 고정한 뒤 최대 조합을 계약 시험으로 측정하고,
  65,536 을 넘으면 기존 최소 전송 잘림 프레임으로 처리한다(§6-2 — 단계적 축소는 후속 후보).
  65,536/65,537 B 경계·전송 잘림 경로 시험은 §7.13 그대로.
...
| 단계 | 사유 |
|---|---|
| 채널 생성 실패 | `child_report_channel_unavailable`(오늘과 똑같이 실행) |
| 파일 없음 | `child_report_missing` |
| 일반 파일 아님·열기 실패·**열기 뒤 읽기 실패** | `child_report_unreadable` |
| 65,537 B 째 읽힘 | `child_report_too_large` |
| UTF-8 디코딩 실패·엄격 JSON 실패(중복 키·NaN·후행 데이터) | `child_report_malformed` |
| 토큰·은행·경로·schema 불일치 | `child_report_mismatch` |

분기:
- **최소 전송 잘림 프레임**(잘림 표지만): 그 형식만 검증 → 통과 시 `child_report_truncated` → 수집·쓰기 unknown. 정상 프레임 필드를 요구하지 않는다
  (요구하면 `invalid` 로 먼저 탈락한다).
- **정상 프레임**: 타입·참조 검증 → 실패 `child_report_invalid`. 포함: ID 중복, 다른 토큰 참조, 존재하지 않는 채택 조회, 정수 자리에 boolean, 필수 통화 누락.
⛔ "첫 실패 하나" 는 **프레임 검증 사유에만**. 실행 결과(timeout·비정상 종료)와 `cleanup_failed` 는 독립적으로 함께 남긴다.
검증 실패를 자식 stderr·로그 재파싱으로 메우지 않는다.

##### 5. 결속과 수락 시험
- 세션: 자식 writer 호출은 `session=child:<토큰>`. 부모 세션 pending 행과 섞지 않는다.
- 회차: 토큰 → round_id 매핑은 부모가 채널 생성 때 기록. 조회 참조 = `(자식 토큰, query_id)`, query_id 0=오늘·1..10=과거.
- woori 요청 날짜는 응답 기준일의 증거로 승격하지 않는다(`date_basis_unconfirmed`, 확인된 위반이 앞선다).
- 수락 시험(명시): ① 부모 세션의 pending 행을 자식 commit 이 확정한 것으로 취급하지 않는다. ② 자식 프레임 유실은 부모의 `no_change_needed`·정책 차단으로
  지워지지 않는다. ③ 부모 `performed` 가 요약으로 선택돼도 자식 유실 사유가 남는다.

##### 6. 결정 (Codex 권고 채택)
1. **상한 단위 = `(자식 토큰, query_id, pair)`** 별 관측 8·miss 8. 잘림 표시·버린 개수·루프 완료 상태도 조회별로 둔다. 충돌은 조회 안에서만 판정.
   근거: 현 상한은 경로×통화 단위라 앞 조회의 miss 10개 뒤 정상 관측에도 `unknown/evidence_incomplete` 가 붙는다(Codex 로컬 재현, miss 상한 검사 bank_report.py:395).
   크기 시험은 **실제 도달 가능한 조합**(과거 순회는 비어 있지 않은 결과를 얻으면 종료 — woori.py:264·344)과 **schema 가 허용하는 최대 조합**을 구분한다.
2. **첫 슬라이스는 기존 전송 잘림만.** 단계적 축소는 후속 후보로만 남긴다. 근거: 충돌을 입증하던 miss 하나·비유한 관측 하나를 빼면 잘림 표시가 있어도
   판정이 `unknown/evidence_incomplete` 로 약해진다(Codex 재현) — 표시만으로 사실 보존이 안 된다. 자식이 판정을 대신 보내는 방식은 "판정은 부모에서만" 과 충돌.
   실제 schema·직렬화 설정 고정 → 크기 측정 → 초과 시 최소 전송 잘림 프레임, 축소 도입 여부는 그 측정 뒤 결정.
...
- (D7) **설계 합의(2026-09-22 Claude·Codex)** — [SOURCE_HEALTH_D7_AGGREGATION.md](SOURCE_HEALTH_D7_AGGREGATION.md). 구현과 shadow 결과 검토 전에는 관리자 API 의미 호환을 주장하지 않는다.
