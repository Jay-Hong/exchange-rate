#!/usr/bin/env python3
"""
source_hourly_rates 테이블 생성 마이그레이션 (ADR-035 D3 / source_hourly_rates Step 1).

배경:
  - v2 1w 그래프 hot path가 읽을 canonical hourly table.
  - source_daily_rates의 1w 대응 (schema mirror, date_kst → bucket_ts_kst).
  - app/models.py의 SourceHourlyRate ORM class 정의 기준.
...
사용법:
  python scripts/migrate_source_hourly_rates.py [--dry-run]

주의:
  - 운영 DB에서 실행 전 반드시 백업 (`scripts/backup-db.sh`).
  - 신규 빈 테이블 추가라 lock 거의 없음, ALTER 없음.
  - 이미 source_hourly_rates 테이블이 있으면 안전하게 skip.
...
def get_dry_run_ddl(dialect_name: str = "postgresql") -> str:
    ...
    table = SourceHourlyRate.__table__
...
def apply_live() -> None:
    ...
    table_name = SourceHourlyRate.__tablename__
    ...
    print(f"  Schema: SourceHourlyRate (ADR-035 D3, app/models.py)")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="source_hourly_rates 테이블 생성 마이그레이션 (ADR-035 D3 Step 1)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
