...
`invalid_token` 에는 만료뿐 아니라 **revoked / disabled user** 가 포함되고(`app/main.py:3101-3108`)
클라는 셋을 구별할 수 없다.
...
`app/main.py:3101-3108`) — `rejectionReason` 만으로 구분되지 않는다. **auth resolution 상태**(refresh 진행 중 / 확정 실패)를 별도로 둬야
표의 두 행이 구현된다.
