...
- 🔲 **ingress 상한(nginx `/ws`) = 미결.** ⚠️ **2026-08-03 확인**: 이 2건은 기능 선행조건(§3.1 표)이
  ...
  **제안서 (값 없음 — 관측이 먼저다)**
  ...
  ⚠️ **실트래픽 규모 실측**(nginx access log, 2026-07-26 ~ 08-04 **9일**):
  `/ws` 항목 **1,698** / distinct 클라 IP **41** / IP 별 누적 연결 상위 **533·533·338·158·26**.
  → **트래픽 자체는 있다**(한때 "실사용자가 없어 baseline 불가"로 적었는데 그건 그 순간의 표본이
  0이었을 뿐이다). 상위 IP 의 533건/9일 ≈ 59건/일 ≈ 24분마다 1회 재연결로, 모바일 백그라운드
  전환의 전형이다 — **handshake rate 근거로는 쓸 수 있다**.
  ⛔ 다만 **누적 연결 수 ≠ 동시 연결 수**다. `limit_conn` 이 필요한 것은 **동시** 분포이고 그건
  access log 가 못 준다(종료 시 기록) — **앱 계측이 그걸 준다**.
  ⛔ 그리고 모바일 IP 는 순환하므로 **41 = 사람 41명이 아니다**. 현재 이용자는 사실상 개발자
  본인 + legacy 소수라, **carrier NAT 꼬리는 아직 대표성이 없다** → `limit_conn` 값은 계속 보류.

  ✅ **해소된 관측**: public smoke 에서 두 연결이 `distinct_ips=1 / max_connections_per_ip=2` 로
  잡힌 것은 **공용 egress 가 아니라** 둘 다 개발자 본인의 같은 머신이었기 때문이다(사용자 확인).
  계측이 정확히 동작한 것이다 — CDN·프록시 수렴을 쫓을 필요 없다. `unknown` 을 실제 IP 처럼 섞으면 carrier NAT 와 구분되지 않아
