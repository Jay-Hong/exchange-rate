...
귀속**시켜야 한다. 그 쌍은 **세 지점 모두**에서 재확인해야 한다 — 어느 하나라도 빠지면 조용히
샌다:

1. **토큰 대기 후 송신 직전** (가장 긴 await 창)
2. **ack / error 처리** (프레임이 구 연결 것일 수 있다)
3. **timeout 처리** (해소 대상이 이미 다음 연결의 슬롯일 수 있다)

⚠️ 그리고 **`cleanup()` 의 순서가 계약이다**: 구 연결의 in-flight 를 **먼저 무효화한 뒤**
소켓을 닫아야 한다. 반대로 하면 close 로 인한 예외가 먼저 올라와, 아직 유효해 보이는 슬롯을
구 task 가 해소하거나 다시 등록할 수 있다.

이 성질들은 *"cleanup 이후 구 token-awaiting task 가 재개해 공유 pending 에 다시 등록하지
못한다"* 형태의 **행동 테스트로** 잠가야 한다 — 구조 검사로는 부족하다. 그 경로는 await 재개
타이밍이라 코드 모양만으로 드러나지 않는다(실측: 서버 쪽에서 같은 종류의 타이밍 성질을
행동 테스트로 잡으려다 실패해 AST 검사로 바꾼 적이 있는데, 클라 쪽은 fake channel 로 재개
시점을 **결정적으로** 제어할 수 있으므로 반대로 행동 테스트가 맞다).
