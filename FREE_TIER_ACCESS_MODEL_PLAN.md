...
- [~] **제품 결정 S6(last-good 최대 stale) = 24h hard cutoff 확정(2026-07-21)** — **서버 절반 구현 완료**(as_of>=24h serve 거부), **iOS 렌더 cutoff 미완**(scheduler asOf+24h wake + `.unavailable`; App Store 출시 전 필수). source-level staleness는 별도 source-health. + validator value-level 완결(entry.rate 타입/point 내부).
