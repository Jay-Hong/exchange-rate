...
> **확정**: IN/BREAK = 10초 1회, OUT = **1분 1회**
> ⚠️ 2026-08-28 정정 — 초기 설계·구현은 OUT 10분이었지만, 이후 `dxy_spot` 독립
> 스케줄로 전환되면서 코드가 매분 `:15`가 되었고 이 문서가 따라가지 못했다.
> 배포 2A에서는 초만 `:15`→`:21`로 이동(KB 뉴스 고정 시작초 분리)했고 주기는 계속 1분이다.

| 모드 | 주기 | 실행 시각 | 비고 |
|------|------|----------|------|
| **IN** | 10초 1회 | 01, 11, 21, 31, 41, 51초 | 운영 broadcast는 매초, 초는 부하 분산 기준 |
| **BREAK1** | 10초 1회 | 01, 11, 21, 31, 41, 51초 | 동일 |
| **BREAK2** | 10초 1회 | 01, 11, 21, 31, 41, 51초 | 동일 |
| **OUT** | **1분 1회** | 매분 21초 | 배포 2A에서 KB 뉴스 `:15`와 고정 시작초 분리 |

> **근거**: 평일에는 환율 그래프 우측 끝 실시간성을 맞추기 위해 10초 주기를 유지한다.
> 주말 OUT도 코드상 계속 1분 주기였으며, 배포 2A는 알려진 고정 작업과 시작초를 분리했다.
...
| 7 | IN/BREAK 크롤링 | 10초 1회 (01/11/21/31/41/51초) | 실시간 비교 품질 향상 |
| 8 | OUT 크롤링 | 1분 1회 (매분 21초) | 배포 2A 고정 시작초 분산 |
