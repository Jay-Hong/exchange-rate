...
> **확정**: IN/BREAK = 10초 1회, OUT = 10분 1회 (운영 반영)

| 모드 | 주기 | 실행 시각 | 비고 |
|------|------|----------|------|
| **IN** | 10초 1회 | 04, 14, 24, 34, 44, 54초 | Broadcasting 전 DXY 갱신 |
| **BREAK1** | 10초 1회 | 04, 14, 24, 34, 44, 54초 | 동일 |
| **BREAK2** | 10초 1회 | 04, 14, 24, 34, 44, 54초 | 동일 |
| **OUT** | **10분 1회** | 4, 14, 24, 34, 44, 54분 44초 | 주말 변동 완만 + 다른 OUT 크롤러와 분산 |

> **근거**: 평일에는 환율 그래프 우측 끝 실시간성을 맞추기 위해 10초 주기로 상향.
> 주말에는 DXY를 10분 간격으로 유지하되, 다른 OUT 모드 크롤러와 충돌하지 않도록 분/초를 분산.
...
| 7 | IN/BREAK 크롤링 | 10초 1회 (04/14/24/34/44/54초) | 실시간 비교 품질 향상 |
| 8 | OUT 크롤링 | 10분 1회 (4/14/24/34/44/54분 44초) | 주말 변동 완만, 다른 크롤러와 분산 |
