...
구현 커밋 단계의 작업은 로컬 구현·시험이었고 서버 설치·크론 변경은 포함하지 않았다 — 설치는 2026-09-23 별도 승인 작업([설치 기록](#설치-기록)).
...
1. 검토된 스크립트의 SHA-256과 호스트 사본을 대조한다. 호스트 `/usr/bin/python3 --version`이
   ...
   **실행 파일은 서버 checkout 이 아니라 Git 밖 `/home/ubuntu/fxi-host-tools/docker_log_archive.py` 에 둔다.**
   서버 checkout 은 배포된 코드와 같아야 하는데(배포 기준선), 설치 당시 운영 checkout(`34ef52c5`)은 이 스크립트를 포함하지 않았다
   (09-29 E0 대상 `b0b44f1` 은 설치본과 같은 바이트를 포함한다 — 그래도 실행은 Git 밖 사본이다). 설치를 위해 checkout 을 옮기면 실행 중인 이미지와 어긋나고,
...
| 항목 | 값 |
| --- | --- |
| 설치 | 2026-09-23 06:20 KST (crontab 적용 완료 `2026-09-22T21:20:19Z`) |
| 설치 대상 커밋 | `cc8162a184013689181f59bccccb15ef135a1972` |
| 스크립트 SHA-256 | `2e831a63f93163cb8dfb16a68c0e458bc37731f3bf8b214e718e6dd3b84eeb20` |
| 설치 경로 | `/home/ubuntu/fxi-host-tools/docker_log_archive.py` (0700, 소유자 ubuntu) |
| 보존 루트 | `/home/ubuntu/logs/docker-archive` |
| 호스트 Python | 3.12.3 |
| crontab 적용 뒤 SHA-256 | `8e62cd00174174d14356715641ac65a27ec49d264883ec638a34f4deb509e6a0` (후보와 동일) |
| 설치 수집 run | `e86b40a82f5d469cb34c4f3033e06921` (복원 verified) |
| 첫 정기 슬롯 | `2026-09-22T21:41:00Z` (run `177c500dc22746a0be3da091560f603b`) |

**설치 목표(위 4단계) 충족 확인**
- 2026-09-25T16:40Z 조회: 정기 run 67개가 예정 슬롯 67개(2026-09-22T21:41Z ~ 2026-09-25T15:41Z)를 누락 없이 채웠고
  크론 출력 67줄 모두 종료 0·만료 실패 없음. `check` 의 `invalid_runs` 가 비어 있다 = 그때까지의 68개 run 전부 두 스트림을
  끝까지 해제해 해시·크기를 대조했다.
- 그 뒤(2026-09-25T16:41Z 슬롯 이후) `restore()` 직접 호출 3건(설치 수집·첫 정기·16:41Z 슬롯 run)도 양쪽 스트림 일치.
  비어 있지 않은 stderr 복원은 설치 수집 run 한 번으로만 확인했다(정기 run 의 stderr 는 0 바이트).
- 실측 증가 29.3 MB/일(65.8시간 표본), 14일 추정 약 410 MB.

점검 명령(읽기 전용):

```bash
/usr/bin/python3 /home/ubuntu/fxi-host-tools/docker_log_archive.py check \
  --root /home/ubuntu/logs/docker-archive --first-slot '2026-09-22T21:41:00Z'
```
...
호스트의 **3.12.3** 실행은 2026-09-23 설치에서 확인했다([설치 기록](#설치-기록) — 67회 정기 실행 종료 0). 위 표의 3.12.14 결과가 3.12.3 을 대신 증명하지는 않는다.
