...
- **IBK 결과 경로 — Selenium 안전망·전체 흐름 검증·운영 배선(기본 OFF)** (2026-09-10, 로컬 구현·미배포): 위 준비 항목의 **후속이자 부분 supersede**. 공식 HTTP 가 기술적으로 실패했을 때만 여는 검증된 Selenium 안전망(`app/ibk_selenium_strict.py` + `app/ibk_selenium_adapter.py`), 부모 작업 기한을 가드 대기·제출·확인 대기·화면 읽기까지 전 구간에 전달, 배선 오류를 브라우저를 열기 전에 실패시키는 사전 검사, 관측 실패 사유 분류 정정(`IbkReason.UNATTRIBUTED_ERROR` — 확정하지 못한 원인을 전송·계약 오류로 적지 않는다), 실제 생성기부터 경보 발송까지 잇는 통합 시험(`tests/test_ibk_end_to_end.py`). **운영 배선은 `IBK_RESULT_PATH_ENABLED`(기본 `false`) 게이트 뒤에 있다** — false 면 `IBK_RESULT_CRAWLER = None`, worker 에 부모 미주입, 경보 전달 객체·스레드 미생성이라 **배포가 곧 동작 변화가 되지 않는다**. 위 준비 항목의 "worker는 인자 없이 생성되어" 는 이 게이트로 대체됐다(false 일 때 동작은 같다). 상태 조회 `GET /admin/api/ibk-result-path`. **활성화는 배포와 분리된 별도 GO** 이며, false 복귀는 legacy 의 MIBANK 저장·기존 재시도 정책도 함께 복원하고 **이미 저장된 데이터는 되돌리지 않는다**. 실제 HTTP·Selenium·운영 PostgreSQL·Telegram 발송 검증은 아직 아니다. [CRAWLERS.md IBK](CRAWLERS.md#ibk-기업은행-appcrawlersibkpy-).
