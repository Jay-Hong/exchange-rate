...
- **배포 2 본단계 1 — 우리은행 IN 30초** (2026-09-01, 배포 전): IN 모드의 `task_woori`만 60초 `:53` → 30초 `:14/:44`로 상향한다. BREAK1은 기존 단일 `OrTrigger`와 `:53`·05:04:53 종료를 유지하고 BREAK2/OUT 제외도 불변이다. 열거된 고정 IN job과 동일 시작초는 없으며, 재기동마다 위상이 달라지는 IntervalTrigger는 고정 레인으로 회피할 수 없으므로 배포 smoke의 `scheduler_event`·응답시간·자원 델타로 판정한다. KB·Hana 주기, 은행 폴백, IBK, DB schema는 변경하지 않는다.
