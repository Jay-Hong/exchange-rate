...
- **우리은행 야간 수집 창 06:00 이동** (2026-09-10, ADR-044): 사용자가 은행 페이지에서 **2026-09-05(토) 05:55:56 고시**를 확인했다. 그 시각은 구 창(`05:04:53` 종료)의 51분 뒤였고, 토요일이라 이후 월요일 08:00까지 **예약된 수집 기회가 없었다**(BREAK1 은 05:04:53 이후 발화 없음, BREAK2·OUT 은 미등록). 운영 DB 는 `09-05 03:55:44` 다음 행이 `09-07 08:00:14` 다 — ⚠️ 세 사실은 별개이고, DB 기록만으로 놓친 고시의 횟수·동일성은 확정할 수 없다. BREAK1 을 `19:00~05:59` `:14/:44`(마지막 **05:59:44**, 1,320회)로 넓히고 BREAK2 에 마무리 조회 `06:00~06:03 :14/:44` + `06:04:53`(화~토, 9회)를 둔다 — 합계 1,329회, 기존 대비 **+120**(예약 횟수이지 포착 보장 아님). ⛔ 마무리 조회는 **같은 `task_woori` ID** 여야 한다: executor 의 `_instances[job.id]` 는 remove/add 로 사라지지 않아 같은 ID 면 전환을 걸친 실행 중 마무리 발화가 차단되고, 다른 ID 면 동시 실행된다(양성 대조 시험). 시간 간격은 해결책이 아니다. ⚠️ **05:55:56 은 "05:00 이후에도 고시한다" 는 관측이지 "06:00 에 끝난다" 는 증거가 아니며**, 06:00 은 우리가 정한 수집 정책이다. ⚠️ **토 06:04:53 ~ 월 08:00 은 미관측 구간**이고 그 구간의 추가 고시 부재를 보장하지 않는다. 다른 은행·모드 경계·OUT 주말 정책·과거 데이터는 불변. [ADR-044](DECISIONS.md#adr-044-우리은행-야간-수집-창을-0600-정책으로-이동--관측과-정책의-분리).
- **IBK 결과 경로 — Selenium 안전망·전체 흐름 검증·운영 배선** (2026-09-10, **배포·활성화 완료**): 아래 준비 항목의 **후속이자 부분 supersede**. 공식 HTTP 가 기술적으로 실패했을 때만 여는 검증된 Selenium 안전망(`app/ibk_selenium_strict.py` + `app/ibk_selenium_adapter.py`), 부모 작업 기한을 가드 대기·제출·확인 대기·화면 읽기까지 전 구간에 전달, 배선 오류를 브라우저를 열기 전에 실패시키는 사전 검사, 관측 실패 사유 분류 정정(`IbkReason.UNATTRIBUTED_ERROR` — 확정하지 못한 원인을 전송·계약 오류로 적지 않는다), 실제 생성기부터 경보 발송까지 잇는 통합 시험(`tests/test_ibk_end_to_end.py`). **운영 배선은 `IBK_RESULT_PATH_ENABLED`(기본 `false`) 게이트 뒤에 있다** — false 면 `IBK_RESULT_CRAWLER = None`, worker 에 부모 미주입, 경보 전달 객체·스레드 미생성이라 **배포가 곧 동작 변화가 되지 않는다**. 아래 준비 항목의 "worker는 인자 없이 생성되어" 는 이 게이트로 대체됐다(false 일 때 동작은 같다). 상태 조회 `GET /admin/api/ibk-result-path`. **활성화는 배포와 분리된 별도 GO 였고 2026-09-10 에 승인·수행됐다** — 코드 배포 `9d4e1c4` 후 운영 `.env` 에 `IBK_RESULT_PATH_ENABLED=true`·`TELEGRAM_ENABLED=true` 를 넣었다. ⛔ **코드 기본값은 여전히 `false`** 라 리포만 보고 운영 상태를 읽으면 안 된다. 활성화 후 관측(각각 독립된 관측이다): 결과별 건수는 `PRESERVED` 30 · `OBSERVED` 82 이고 `DEGRADED`·`FAILED` 는 계수 자체가 없었다. 부모의 `notice_enqueued`·`notice_enqueue_failed`·`notice_suppressed` 가 모두 0 이고 전달 측 접수도 0 이다. `IBK_SELENIUM_NET_*` 마커는 0건이다. ⛔ 접수 0 만으로 장애 부재를 주장하지 않는다 — 접수에 실패해도 전달 측 접수 수는 0으로 남을 수 있으므로 결과별 건수와 enqueue 실패 계수를 함께 봐야 한다. false 복귀는 legacy 의 MIBANK 저장·기존 재시도 정책도 함께 복원하고 **이미 저장된 데이터는 되돌리지 않는다**. 실제 공식 HTTP·운영 PostgreSQL 은 활성화 관찰로 확인했고, Telegram 은 별도 점검 발송 1건으로 모듈까지만 확인했다(운영 부모 경보 경로의 자연 발생 실증은 아직 없다). **실제 장애에서의 Selenium 안전망 발화도 아직 관측되지 않았다** — 시험 범위와 별개로, 그 관찰 창에서 해당 경로의 운영 실행 증거를 얻지 못했다는 뜻이다. [CRAWLERS.md IBK](CRAWLERS.md#ibk-기업은행-appcrawlersibkpy-).
- **IBK outcome 준비 — 결과 프로토콜·판정 어댑터·미연결 생성기** (2026-09-07, 로컬 구현·미배포): **준비 구현과 해당 범위 검증을 마친 부분** — 기존 종료 분기 타입화(`IbkLegacyResult`), 결과 프로토콜(`app/ibk_result_protocol.py`), 유한 subprocess capture(`app/ibk_subprocess_capture.py`), 부모가 정한 실행 ID·기준시각(`app/ibk_run_context.py`), 선택적 부모 배선(`app/ibk_parent_runner.py` + `scripts/ibk_subprocess_bootstrap.py` + runner 결과 채널), 판정 어댑터(`app/ibk_result_builder.py`), 회귀 가드 순수 함수 추출(`app/ibk_regression_guard.py`), 단일 서비스일 결과 생성기(`produce_ibk_dated_result`). **기본 운영 경로는 미전환이다** — `IBK_RESULT_CRAWLER = None`, worker는 인자 없이 생성되어 기존 `execute_with_timeout`을 쓰고, 생성기의 운영 호출자는 없다. GET/POST 순서, Selenium 3회 재시도, MIBANK 저장, 부모 성공/실패 계측은 불변이다. **후속 작업** — 전체 IBK 실행 흐름 연결, Selenium 안전화, POST-first 정책 전환, 실제 경보 연결. 사용자 승인된 IBK MIBANK no-write 정책도 그 단계에서 계측·경보와 함께 반영한다. [CRAWLERS.md IBK](CRAWLERS.md#ibk-기업은행-appcrawlersibkpy-).
