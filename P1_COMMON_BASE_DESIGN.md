...
**🎯 현재 상태 (2026-06-21 갱신)**: dormant-landable 전부 LAND·CI green — W1(`d7a50f8`)·W2(`4909ebd`)·C6-PRE-build-a lease(`3aeac79`)·b1 drain-proof helper(`4f10508`)·b2 migration gate(`29bab06`)·docstring critic#7(`b236435`). **잔존 = 전부 must-confirm(production-irreversible)**: W3(arming) → C6-FLIP-RELEASE(IMAGE_MAX→2) → G2a → STAGE → EXEC-1~8 → C7. (아래 본문은 Step1 작성 시점 설계 기록 + land 마커.)

**중심 사실(검증)**: 현 이미지로 flip **구조적 불가** — `IMAGE_MAX_WRITER_PROTOCOL=1`(atomic_write_control.py:43) ∧ begin-atomic gate가 `required_protocol≤IMAGE_MAX` 요구(activate_atomic_fx.py:711) → `--required-protocol 2` 거부. + **원래 5 gap 중 4개 LAND, W3 + IMAGE_MAX만 잔존**: ✅ W1 `_do_halt`→cas_open_quiesce_session(`d7a50f8`) / ✅ W2 `_do_begin_atomic`→cas_consume_quiesce_session(`4909ebd`) / ✅ lease brick cas_acquire/renew/release(`3aeac79`) / ✅ prod migration 경로 `--allow-production-migration`(`29bab06`, drain proof+lease+backstop) — 전부 dormant land·CI green. **잔존 must-confirm**: W3 main()→RealQuiesceBoundary 미주입(현재 `_FailClosedQuiesceBoundary` stub :380/:852) + IMAGE_MAX_WRITER_PROTOCOL=1(:43 → RELEASE에서 2).

**재분류(pivot)**: "남은 건 전부 must-confirm"은 부정확 — **planning-only prerequisite**(코드/prod 변경 0): ✅ W0(session-id 계약 LOCK, codex 019ee2ee). **dormant-landable ✅ 전부 LAND (2026-06-21, behavior-change-0·CI green)**: W1(`d7a50f8`)·W2(`4909ebd`)·C6-PRE-build-a lease(`3aeac79`)·b1 drain-proof helper(`4f10508`)·b2 migration gate(`29bab06`)·docstring(`b236435`). **잔존 must-confirm**: W3(arming)·C6-FLIP-RELEASE(IMAGE_MAX bump)·G2a·STAGE·모든 EXEC·C7.
...
- **[must-confirm]** **W3**(main():852 RealQuiesceBoundary 주입 + trip-wire test_main_does_not_inject_real_boundary **invert**; legacy steady-state엔 inert이나 arming step) → **C6-FLIP-RELEASE**(IMAGE_MAX→2 atomic_write_control.py:43; W3 land 후 동반 ship; deploy 시 production-visible — begin-atomic 구조적 가능해짐)
...
**where dormant ends**: ✅ W1/W2/C6-PRE-a(lease)/b1/b2/docstring LAND dormant (2026-06-21, CI green). **W3가 다음 단계** = inflection(main() edit + trip-wire invert = arming, legacy엔 inert이나 실 quiesce flow arm). C6-FLIP-RELEASE에서 begin-atomic 구조적 가능. EXEC-1부터 live production.
