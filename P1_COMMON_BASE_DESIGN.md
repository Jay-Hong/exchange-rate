...
**중심 사실(검증)**: 현 이미지로 flip **구조적 불가** — `IMAGE_MAX_WRITER_PROTOCOL=1`(atomic_write_control.py:43) ∧ begin-atomic gate가 `required_protocol≤IMAGE_MAX` 요구(activate_atomic_fx.py:711) → `--required-protocol 2` 거부. + **5 gap(dormant 아닌 미구현)**: W1 `_do_halt`→cas_open_quiesce_session 미호출(:476) / W2 `_do_begin_atomic`→cas_consume_quiesce_session 미연결(:495) / W3 main()→RealQuiesceBoundary 미주입(현재 `_FailClosedQuiesceBoundary` stub :380/:852) / lease brick(cas_acquire/renew/release) 부재(컬럼·CHECK만 models.py:348-365) / prod migration 경로 부재(atomic_migration.py:483-509 hard-block, override 없음).

**재분류(pivot)**: "남은 건 전부 must-confirm"은 부정확 — **planning-only prerequisite**(코드/prod 변경 0): W0(session-id 계약 설계). **dormant-landable**: W1·W2·C6-PRE-build-a(lease brick)·C6-PRE-build-b(migration flag)·docstring 정정 (scripts/ 미실행 + uncalled 함수/flag, live importer 0 → behavior-change-0). **must-confirm**: W3(arming)·C6-FLIP-RELEASE(IMAGE_MAX bump)·G2a·STAGE·모든 EXEC.
...
- **[dormant-landable]** **W1**(`_do_halt`+cas_open_quiesce_session **same tx**, W0 계약대로) → **W2**(`_do_begin_atomic`+find_open_quiesce_session+cas_consume + cutover bootstrap=discovered + optional cross-check, same tx) + **C6-PRE-build-a**(lease brick, **DB-clock 권장**+TTL>migration 최악) → **C6-PRE-build-b**(`--allow-production-migration` flag: halt-mode 확인[requested_mode==HALT @expected-gen] ∧ lease 획득 시만 check_apply_safety 우회; 기존 sqlite/localhost 경로·`_check_apply_target` backstop 보존) + **docstring 정정**(activate_atomic_fx.py:20-21 "atomic=write 0"→실제 v2 write; **보완 B — RELEASE에서 분리해 dormant 단계로 앞당김**, correctness-trap 조기 제거)
...
**where dormant ends**: W1/W2/C6-PRE-a/C6-PRE-b/docstring land dormant. W3가 inflection(main() edit + trip-wire invert = arming, legacy엔 inert). C6-FLIP-RELEASE에서 begin-atomic 구조적 가능. EXEC-1부터 live production.
