...
- **[must-confirm, 비가역]** **EXEC-1** RDS snapshot+halt(legacy→halt, FX publish gate close, baseline WS 무영향 :650) → **EXEC-2** recreate-drain(flip-release image, fresh process가 durable halt ACK record_app_ack_if_quiescing main.py:543-544) → **EXEC-3** C6-PRE migration(halt window 안, Option B no-race, `--scope all`=FX-all[latest:source:* selector 부재]) → **EXEC-4** verify(dry-run all already_current + verify-only 3 asset; conflict/invalid_schema→STOP) → **EXEC-5 begin-atomic(★THE FLIP: halt→atomic + cutover idle→running 1 tx, activation_epoch=1 one-way door)** → **EXEC-6** recreate-atomic(writers atomic + cache refresh §9:90) → **EXEC-7** verify(running→verified) → **EXEC-8** finalize(asset ready×3 먼저 + verified→completed 마지막 1 tx, **★FX publish gate OPEN**)
