...
- **usdt/krx DB-side full halt**: A2-3는 Redis-side writer만 gate(usdt 382 / krx tick 624 / krx routine flag=false). DB writer(`insert_source_rate_if_changed`, usdt WS DB writer + KrxDbWriter)는 미gate라 halt 시 DB write는 진행(Redis만 차단 = benign partial, read-path DB-fallback). bank/investing(A2-2)은 orchestrator top gate라 DB+Redis 동시 정지인 것과 비대칭 — usdt/krx의 DB-side 정지(완전 halt/atomic quiesce)는 C6에서 atomic DB+Redis 단위로 결정.
  ...
  **write-mode gating topology (C6-5b 완료 기준):**
  ...
  | usdt/krx routine tick | **ungated** (항상 write) | gated → BLOCKED | A2-3 (Redis writer만) | DB mode-indep / Redis mode-dep |
  | usdt/krx close finalizer | **ungated** (`insert_source_rate_unconditional`) | `set_latest_krx` C6-5b-2 gate → halt/atomic False | §389 close-path | DB mode-indep / Redis mode-dep |
  ...
  **원칙**: Redis latest-side가 §11 FX atomic invariant를 소유한다. usdt/krx DB canonical은 mode-independent로 유지 — atomic flip 하에서도 history 보존 + Redis miss/evict 시 유일 fresh recovery rail. usdt/krx 자체 cutover만이 DB+Redis 단위 retrofit으로 이 비대칭을 변경한다 (신규 섹션 금지 — 본 resolution 블록이 단일 ledger).
...
6. **USDT/KRX global freeze 비대칭(critic #4)**: flip 후 latest:source:* un-migrated v1 잔존(안 읽힘, §393-394 안전) — operator ack 필요("migration 미완" 오독 방지). `--ack-global-writer-mode-scope`(:696) 동반.
7. **in-flight straddling halt commit(critic #10)**: low-churn window + **KRX close-finalizer window 회피**(~15:35-15:50 KST CF, ~05:50-06:05 CM — KRX는 FX scope 밖이나 global freeze가 KRX close write 건드려 quiesce/ACK telemetry attribution 흐림).
