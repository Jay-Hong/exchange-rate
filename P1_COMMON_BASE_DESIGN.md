# P1 공통 base 구현 설계 — control plane + rollout (design + A1~A5 land + B1·B2a land)

> ⚠️ **상태**: 설계 분석 + **A1~A5 dormant land (2026-06) + A-series holistic 점검**: A1 control plane / A2 writer mode-aware gate + revision plumbing / A3 v2 schema + Lua atomic primitive + migration command(runner+CLI) / A4 WriteOutcome+PendingCandidate interface / A5 cutover state + publisher gate skeleton — **전부 dormant, behavior-change-0, live write 미배선**. **B1(success watermark schema + serialization + pure relation classifier) dormant land (2026-06-18)** — plan-first 단계에서 PR_D §12 대조로 재정의(arbitration/seq 발급/bootstrap 결정/eviction-alive 연속성/completion gate는 B2b coordinator 소유; B1은 pure 분류기). **B2a(BuildResult + dormant builder wrapper, `atomic_build.py`+`fx_membership.py`) dormant land (2026-06-18)** — Workflow가 틀린 builder anchor 적발 → 실제 fx builder(`load_and_build_fx_topic_payload`) 기반 재정의, revision vector는 B2b 전담(v2 결합). 경계·control plane·**outcome 계약(§14)·cutover state machine(§15)·revision-ID 확보(§16)·atomic primitive Lua(§17)·P1b/D 구현 분해(§19) 확정**. **P1 설계 + 구현 분해 전부 resolved**(§13). 잔여 = B2b/B3 + C6/C7 + 별도 client/server 계약(subscribe-time initial snapshot).
...
- admin 변경(atomic↔halt) = **activation FLIP quiesce(§9 step 6)와 별도 runtime 전환**: 공통 불변 = `mode_generation` bump + **durable DB 반영** + cache 확인 + durable ACK (단순 "DB write + cache 갱신" 아님). drain 메커니즘은 분리 — **activation FLIP은 recreate + fresh-process halt ACK**(§9/§19 C6-quiesce); admin in-place atomic↔halt의 in-flight 처리는 별도(incident atomic→halt는 in-flight v2 write 완료가 무해[valid v2]라 cache-refresh로 신규만 차단)
- **`mode_generation` fencing (진행 중 write 동기화, must)**: write 시작 시 (in-process) `mode_generation` 읽고 허용 → 전환 시 bump + **신규 write 차단** + 진행 중 write **drain** → 전환 완료 ACK. (⚠️ activation FLIP quiesce의 drain 메커니즘은 **recreate + fresh-process halt ACK**로 개정 — in-process counter는 Selenium subprocess writer 누락 + cross-process 불가, §9 step 6 / §19 C6-quiesce. `mode_generation` fencing token 자체는 유지 — boundary가 `mode_generation>=halt_gen`로 stale 관측 차단.) ⚠️ **Lua script로 `mode_generation` fencing은 불가** (Lua는 Redis만 보고 DB generation 못 읽음; Redis 복제는 eviction/sync 재발) → **activation FLIP의 in-flight write 정리 = recreate + fresh-process halt ACK로 확정**(단일 프로세스 전제, 위 ⚠️ — 구 '공통 gate+drain' 개정, §9 step 6 / §19 C6-quiesce; admin in-place 전환은 별도), 다중 프로세스 script fencing은 별도 fence-passing 계약 = 범위 밖. (§11 revision compare-write의 Lua/WATCH는 revision이 Redis value에 있어 별개.)
- 주기 poll로 외부 변경 동기화
...
6. **live-app quiesce handshake** (명시 control endpoint/command, DB→poll 아님) — **durable 순서**: (a) **DB tx: `requested_mode=halt` + `mode_generation` 증가** (durable, §9:88) → (b) **app recreate** (old container 완전 종료 → main thread + Selenium subprocess + Chrome child 전부 정리) → (c) **fresh process가 durable halt 관측** (startup refresh: `requested_mode=halt` ∧ `enforced_action==HALT` ∧ `mode_generation>=halt_gen`) → (d) **fresh-process halt ACK durable 기록** (`process_started_at>halt_committed_at` 포함) → (e) **boundary가 DB ACK 검증** → begin-atomic. **drain = recreate + fresh-process halt ACK** (구 'in-process counter/lock drain' **개정** — in-process counter는 Selenium subprocess FX writer[shinhan/ibk/nh/sc는 runner.py child process에서 `insert_bank_rates_into_db`] 누락 + 별도 activation process가 live counter 못 읽어 단독 불충분. §19 C6-quiesce 결정.)
...
⚠️ **activation race (must, blocker)**: 실행 앱을 legacy-cached로 둔 채 DB mode만 atomic으로 바꾸면 poll 전까지 unconditional write 계속 → **activation 순간 invariant 깨짐**. → 위 6–8 (durable halt-quiesce[**drain = recreate + fresh-process halt ACK**, §19 C6-quiesce] → one-shot → recreate-atomic). 최초 활성화 = **`legacy → halt → atomic` composite** (§3).
...
- per-asset: `asset` / `publish_state = blocked | ready` / `ready_revision_vector`(DB-present sources, partial-tolerant) / `membership_version`(source_registry **또는 별도 고정 계약**, §5.1 line 65 — FX는 source_registry가 9은행/jpy/eur 미모델링이라 **고정 계약 `fx_membership.py`**: BANK_DISPLAY_ORDER+investing / FX_MEMBERSHIP_VERSION)
...
- **B1 Redis success watermark** (non-durable; eviction/비호환 → bootstrap):
  ...
  - **✅ land (2026-06-18, dormant, `app/atomic_watermark.py`)** — plan-first에서 Workflow 4-lens + codex(xHigh/high) 의견일치로 **재정의**: B1 = **pure schema + serialization + relation classifier**만. **B2b coordinator로 defer**(B1 미포함): lineage **arbitration**(B1은 `LINEAGE_MISMATCH`만 반환 — bootstrap-wins는 B2b가 durable control plane active session/generation+lease[§15 line 194/§18] 대조로, 단순 wins 아님) / `publish_sequence` 발급·monotonicity(coordinator in-memory, eviction-alive 연속성) / completion gate(sent_count>0 후 write) / bootstrap 결정. **wall-clock epoch ordering 제거**(시계 역행 취약) — lineage_id는 **opaque str**(B2b가 session_id:generation 등 조합), epoch 필드 없음. 구현: `Watermark`(frozen, `__post_init__` 단일 invariant 강제 + missing_sources 정렬 canonicalize + present_revision_vector read-only) / `serialize/deserialize_watermark`(fail-closed `WatermarkSchemaError` → 호출자 bootstrap) / `watermark_content_equal`(§12 line 219 vector-whole identity — B3 dedup·sent_but_uncommitted idempotency, asset mismatch→ValueError) / `classify_watermark_relation`(NO_CURRENT/LINEAGE_MISMATCH/NEWER/OLDER/SAME/SAME_SEQ_CONTENT_DIVERGENT) / `is_watermark_compatible`(membership/schema predicate, type-strict) / `watermark_key`. present_revision_vector = **effective-only**(None 금지; skipped_newer effective 확보는 B2b §15 line 215) + `atomic_value_schema.{make_revision_key_from_revision, parse_revision_key}` 단일 인코딩.
- **B2a Builder BuildResult**: `{asset, payload, present_sources, missing_sources, membership_version, completeness[complete|partial|malformed], build_error}`. (구 sketch의 `present_revision_vector`는 land 시 **B2b로 이동** — 아래 land 노트 참조. effective revision은 v2 결합이라 B2a 미포함.)
  - **partial publish 허용**(current compat — legacy partial-publish 보존), missing_sources = telemetry/reconciliation input, **strict all-source block = future policy**. payload엔 revision/id 미노출(§5.1 line 70 — public WS payload revision 내부 전용; 구 §12.7 인용은 오기).
  - **✅ land (2026-06-18, dormant, `app/atomic_build.py` + `app/fx_membership.py`)** — plan-first에서 Workflow 4-lens가 **틀린 builder anchor(build_rates_payload)** 적발 → 재정의(codex Option B 의견일치): 실제 fx builder = `load_and_build_fx_topic_payload`(Redis-first per-source, schema-v1 snapshot, PR_D line 16). **revision vector는 B2a 미포함 → B2b 전담**: §12 line 70의 effective revision vector는 payload entry에 revision 필요(=v2 활성). dormant(v1) DB candidate vector는 payload(Redis-first)와 다른 snapshot이라 built_revision 아님 → v2 활성 후 builder가 같은 Redis read에서 revision_key 추출(B2b/C6). B1 watermark effective-only와 정합(placeholder vector 금지). 구현: `BuildResult{asset, payload(Optional[dict] schema-v1 — MALFORMED 시 None, codex), present_sources, missing_sources, membership_version, completeness(COMPLETE/PARTIAL/MALFORMED), build_error}` + `__post_init__`(partition/payload-match invariant) + `build_fx_result(db, asset)`(loader wrap → present/missing/completeness, 예외→MALFORMED, invalid asset→ValueError). **membership = 별도 고정 계약**(`fx_membership.py`: BANK_DISPLAY_ORDER+investing, FX_MEMBERSHIP_VERSION 명시 int + lock-test — source_registry는 FX 은행 미모델링). completeness 책임 경계: builder는 present/missing만, no-DB-value vs fallback-fail 분류는 B2b DERIVED pending(§19 아래). **non-island**(atomic_value_schema 미import)이나 dormancy 테스트 skip엔 포함(docstring이 dormant siblings 언급). live(fx_topic_payload/crud/fx_membership)만 import, live caller 0.
...
- **A2-3 (usdt/krx Redis writer 분기)**: routine tick writer **2개만** gate — `set_latest_usdt_rate_from_sync_job` + `set_latest_krx_rate_from_sync_job_tick_level`. 최상단 snapshot 1-read(Redis I/O 전) → halt/atomic은 **`BLOCKED` outcome**(coalesce `SKIPPED`와 구분 — polling caller success 오인 차단[codex plan-review Medium], trigger 차단; **atomic은 A3 전 fail-closed**, legacy fallback 금지). legacy → 기존 흐름 불변. routine KRX는 prod(`KRX_REDIS_TICK_WRITE_ENABLED=true`)=tick-level(624) + rollback(false)=`KrxDbWriter._sync_db_write` flag=false 분기(`write_after_db_insert`→571)도 gate(두 config 모두 Redis-side gated, benign partial — DB insert는 진행, read-path DB-fallback, codex xHigh Medium). **`set_latest_krx`(DB-bound, 571) 자체는 gate 제외** — `KrxCloseWindowWriter`/REST close(571 직접, 2308/2880)가 공유라 gate 시 close finalizer partial → deferred (→ **C6-5b-2에서 gated**: direct/shared KRX setter top-gate, halt/atomic→False; close finalizer는 Redis-side block + DB-side mode-independent로 정리, §389/§392). **usdt/krx DB-side full halt**(`insert_source_rate_if_changed` gating)는 C6 — A2-3는 Redis-side만 (→ **C6-5b-5에서 DB-side mode-independent 유지로 closed**, §392). mirror(`_mirror_all_latest`)는 A2-3 시점 미연결(AST trip-wire로 잠금)이었으나 **C6-5b-4에서 atomic 연결 완료**(write-mode gate + bank/investing v2, line 387 land — negative trip-wire는 positive characterization으로 교체). approximate block counter + usdt_sources polling caller BLOCKED 처리. **→ 2026-06-22 decouple hotfix가 이 usdt/krx Redis gate를 전부 제거(§401-405 Amendment) — 본 A2-3 gate 기술은 historical(land 기록); 현재 usdt/krx Redis writer는 mode-independent.**
...
- **mirror atomicization** ✅ **[C6-5b-4 land]**: `_mirror_all_latest`가 또 다른 latest:* writer → atomic 시 v1 계속 쓰면 §11 invariant 깸. atomic 활성 전 mirror도 atomic primitive 공유. **구현**: `_mirror_all_latest` 최상단 `enforced_action` 1-snapshot gate — ATOMIC → bank/investing data key를 §16 re-read revision selector(direct flush-row-ref와 same row→same revision) + v2 compare_write(island `build_atomic_writer`/`atomic_compare_write_v2`, async `_mirror_all_latest` 안에서 단일 `asyncio.to_thread`로 sync 루프, writer 1회 생성) / HALT(및 예상 밖) → §9 quiesce full skip(`_mirror_skip_stats`, bank/investing/index/DXY 전부) / LEGACY → 기존 본문 byte-identical(early-branch만 위에 얹음). outcome→stats: advance/refreshed_equal/skipped_newer=loaded(+index) / conflict·structural·general=failed(index 게이트 `failed==0` 차단). **DXY는 v1 유지**(별도 카테고리, latest:index 무관), **source 루프는 allowlist skip**(USDT/KRX source atomic은 line 386 별 트랙), **latest:index는 v1 serialize_index**(control key, revision 없음). mirror는 **topic trigger 미발사**(value-recovery/refresh). island helper `present_for_index`/`outcome_label`로 `latest_rates_cache`가 atomic primitive 직접 import 0(C7 경계). atomic 분기는 C6-FLIP까지 dormant(prod LEGACY). dormant land — behavior-change-0.
...
- **usdt/krx DB-side full halt**: A2-3는 Redis-side writer만 gate(usdt 382 / krx tick 624 / krx routine flag=false). DB writer(`insert_source_rate_if_changed`, usdt WS DB writer + KrxDbWriter)는 미gate라 halt 시 DB write는 진행(Redis만 차단 = benign partial, read-path DB-fallback). bank/investing(A2-2)은 orchestrator top gate라 DB+Redis 동시 정지인 것과 비대칭 — usdt/krx의 DB-side 정지(완전 halt/atomic quiesce)는 C6에서 atomic DB+Redis 단위로 결정.
  ...
  ✅ **[C6-5b-5 land — keep USDT/KRX DB-side MODE-INDEPENDENT]**: 이 open decision을 닫는다 — usdt/krx DB writer(`insert_source_rate_if_changed` crud.py:2265 / `insert_source_rate_unconditional` crud.py:2222; 5 USDT WS `_sync_db_write` + KrxDbWriter/KrxCloseWindowWriter/REST close가 위임)는 **write-mode 미참조 → 모든 모드(legacy/halt/atomic)에서 DB write 진행**(현 코드 그대로 = behavior-change-0). bank/investing과 달리 orchestrator-top gate **없음**. **근거(load-bearing, benign 아님)**: atomic flip 시 usdt/krx `latest:source:*`는 A2-3/C6-5b-2 gate로 BLOCKED + mirror(§387) source allowlist-skip → topic-only consumer는 Redis-first→DB-fallback(`usdt_topic_payload.py:281/:327`). **Redis key가 miss/evict/parse-fail 시 DB canonical이 유일 fresh recovery rail**(allkeys-lru 100mb no-TTL key는 long flip 중 evict 가능 → miss → DB fallback). DB-side까지 막으면 usdt/krx 전면 freeze + 복구 rail 소멸. **알려진 결과(corruption 아님)**: usdt/krx topic read는 stale 가드 없음(`usdt_topic_payload.py:286` "miss 처리만 DB fallback" / `:328` "stale 가드 없음") → key 생존 동안 flip-instant frozen 값 serve(freshness ceiling — present-stale Redis가 fresh DB를 가림, evict/flip 종료까지). **no-corruption**: §11 v2 invariant는 Redis FX/index key에만(§386/§389), `source_rates` DB는 insert-if-changed history store(compare-write invariant 없음); 실제 corruption vector(atomic 중 `latest:source:*`에 v1 SET)는 Redis-side라 C6-5b-2가 차단. usdt/krx 자체 Redis atomic cutover는 별 track(§386 — `_select_latest_source_with_revision` + migration `--scope source` 부재라 **FX flip scope 밖**); 그때 DB+Redis 동시 retrofit과 함께 이 결정 명시 갱신. lock: `tests/test_c6_5b_5_db_mode_independent.py`(legacy/halt/atomic 전수 real-DB write + 두 DB writer body write-mode 심볼 0 AST trip-wire). dormant doc/test land — behavior-change-0. **→ 2026-06-22 decouple: Redis-side도 mode-independent로 정렬(§401-405 Amendment). DB-side mode-independent 결정은 불변·유효; 단 'Redis가 BLOCKED라 DB가 유일 fresh recovery rail'이던 근거는 'DB·Redis 둘 다 mode-independent'로 갱신 — latest:source:*는 FX atomic과 disjoint keyspace라 §11 invariant 무관.**

  **write-mode gating topology (C6-5b 완료 기준):**

  | writer 표면 | DB-side | Redis latest-side | gate 위치 | mode 의존 |
  | --- | --- | --- | --- | --- |
  | bank / investing | gated (halt→skip+return 0 / atomic→v2 commit) | gated (동일 분기) | crud.py:488 / :675 (orchestrator-top) | **MODE-DEPENDENT** (DB+Redis 단위) |
  | usdt/krx routine tick | **ungated** (항상 write) | **ungated** (항상 v1 write) | — (2026-06-22 decouple, 구 A2-3 gate 제거) | **MODE-INDEPENDENT** (DB+Redis 둘 다) |
  | usdt/krx close finalizer | **ungated** (`insert_source_rate_unconditional`) | **ungated** (`set_latest_krx` 항상 write) | — (2026-06-22 decouple, 구 C6-5b-2 gate 제거) | **MODE-INDEPENDENT** (DB+Redis 둘 다) |
  | mirror (`_mirror_all_latest`) | n/a (Redis-only writer) | bank·inv=atomic v2 / usdt·krx=skip / DXY·index=v1 | §387 (C6-5b-4) | bank/inv mode-dep / source skip |

  **원칙**: Redis latest-side가 §11 FX atomic invariant를 소유하나, **그 invariant는 FX(bank/investing)+index+mirror Redis key에만** 적용된다. usdt/krx `latest:source:*`는 FX atomic loader/mirror가 안 읽고 안 쓰는 **disjoint keyspace**라 §11 invariant 밖 → DB·Redis 둘 다 mode-independent가 정합(2026-06-22 decouple hotfix). usdt/krx 자체 cutover가 DB+Redis v2 retrofit + per-source control gate를 도입하기 전까지는 전역 FX writer mode와 무관하게 항상 write (신규 섹션 금지 — 본 resolution 블록이 단일 ledger).

  **⚠️ Amendment (2026-06-22 decouple hotfix)**: FX atomic cutover의 전역 writer mode가 usdt/krx Redis writer까지 BLOCK해 테더 topic trigger를 막아 iOS 테스트 탭을 freeze시킴(SET-only trigger → BLOCKED → no push). **4 gate 제거**(usdt setter / krx base 598 / krx tick 675 / `krx_kis._sync_db_write` flag=false 분기)로 usdt/krx Redis-side를 **DB-side와 동일하게 mode-independent**로 정렬. 안전: latest:source:*는 FX atomic keyspace와 disjoint(atomic loader는 bank/investing만 read, mirror는 source allowlist-skip[source_skipped=0]) → v1 SET이 §11 FX invariant 깰 경로 없음. **전제: 단일 프로세스(`--workers 1`, Dockerfile:118)** — `_last_written_usdt_state`/`_last_written_krx_state` in-memory 가드(coalesce/regression-skip)가 load-bearing이라 multi-process/worker로 가면 disjoint-mode-independent 추론에 Redis-side fence 필요(그때 재평가 — §386 self-cutover track). lock: `tests/test_atomic_write_a2_3.py`(3 setter + `_sync_db_write` 전수 mode-independent + AST trip-wire). 구 BLOCKED enum + `_record_write_mode_block` helper는 dormant scaffolding 보존(per-source cutover gate용).

- **C6-quiesce (live-app quiesce drain = recreate + fresh-process halt ACK)** [plan-locked 2026-06-20 → ✅ **Q0/Q2a/Q2b/Q4a/Q4b ALL LAND 2026-06-20** (dormant building blocks 완료, CI green); operational wiring: ✅ W1(cas_open, `d7a50f8`)·W2(cas_consume, `4909ebd`)·**W3(RealQuiesceBoundary 주입, `df174ac`) ALL LAND**(W1/W2 dormant + W3 arming[prod behavior-change-0], 2026-06-21) + **RELEASE(IMAGE_MAX 1→2 코드 LAND)** — DEPLOY(IMAGE_MAX=2 image)부터 must-confirm. 아래 **C6-FLIP Activation Runbook** 참조]: §9 step 6 quiesce의 drain 메커니즘 확정. **결정: in-process in-flight counter 폐기, recreate-is-the-drain.** 근거(전부 코드 검증): (1) FX writer **multi-thread**(`AsyncIOScheduler(timezone=KST)` executors 미지정 → 기본 ThreadPoolExecutor, scheduler.py:41; sync `wrapper()` :264; job별 `max_instances=1`라 bank job 간 병렬) (2) **Selenium FX bank 4종(shinhan/ibk/nh/sc)이 runner.py child process에서 `insert_bank_rates_into_db` 호출**(runner.py:61-65 + shinhan.py:103/ibk.py:202/nh.py:94/sc.py:106) → main-process counter가 못 세어 **단독 불충분** (3) activation script(activate_atomic_fx.py:776 자체 `SessionLocal`)는 **별도 process**라 live counter 못 읽음(cross-process false-quiesce) (4) **recreate가 이미 runbook**(activate_atomic_fx.py:28/:730; 구 'in-flight drain' 주석은 ✅ W3 sweep에서 정리됨 — grep 'in-flight' = 0) — main thread + Selenium subprocess + Chrome child + queue worker 전부 정리 + fresh process가 startup refresh로 durable halt 관측. **load-bearing chain**: DB halt durable commit(`cas_request_halt`, §9:88) → old container 완전 종료 → fresh process가 `requested_mode=halt` 관측 + `enforced_action==HALT` ∧ `mode_generation>=halt_gen` ∧ `process_started_at>halt_committed_at` → **durable ACK** → boundary가 DB ACK 검증 → begin-atomic. **fresh-process ACK 필수(codex 보강)**: `atomic_write_runtime._INITIAL`은 `enforced_action=LEGACY`(atomic_write_runtime.py:84)라 'recreate 성공'만으론 부족 — startup refresh DB-read 실패 시 legacy passthrough 잔존 가능 → boundary가 **실제 halt 관측 ACK**를 확인해야 안전. **torn mid-write self-heal**: DB commit이 Redis 선행(crud.py:505/:440) + migration이 DB-derived v2 write(Redis 값 copy 아님, atomic_migration.py:7) + post-flip atomic mirror가 DB latest로 Redis v2 재작성(`_mirror_all_latest_atomic` latest_rates_cache.py:1567 → `_sync_atomic_mirror_fx` v2 write :1546; :1713은 legacy mirror라 무관) → recreate가 mid-write를 kill해 생긴 DB-ahead/Redis-stale은 migration·mirror로 수렴. **scope = Q2 + Q4 (pure dormant, hot-path 무접촉, byte-identical)**: **Q2** durable evidence schema/CAS(`atomic_quiesce_session`{session_id/halt_mode_generation/halt_committed_at/state} + `atomic_quiesce_app_ack`{session_id/boot_id/process_started_at/observed_writer_generation/observed_enforced_action/optional queue_size}, A1/C6-8a 패턴 — migrate script + create_all 제외 + CAS brick). **Q4** RealQuiesceBoundary(standalone script가 **DB ACK만 read**해 판정) + live-app startup ACK-writer(active quiesce session ∧ process_started_at>halt_committed_at ∧ enforced_action==HALT ∧ mode_generation>=halt_gen일 때만 ACK write — 조건부, hot-path 아닌 startup-time). default `_FailClosedQuiesceBoundary` 유지, RealBoundary 주입은 **C6-FLIP(W3에서 land 완료, `df174ac`)**. **counter는 폐기**(미래 no-downtime in-place quiesce용 optional adjunct지 FLIP safety proof 아님). **Selenium queue empty는 write-quiesce 필수조건 아님**(fresh process의 subprocess가 runner 진입 refresh로 halt gate-skip) — optional operational quietness(ACK queue_size). plan-first 5-lens Workflow + codex 2-thread 합의(019ee1c2 + 재확인). 순서 **Q0(본 §9/§7 amend) → Q2 → Q4**. activate_atomic_fx.py의 구 "in-flight drain" 주석은 ✅ W3 sweep에서 정리됨. **잔존 must-confirm**(⚠️historical — 2026-06-21~22 전부 실행 완료, 잔존=C7 only, 418 헤더 참조): DEPLOY(IMAGE_MAX=2 image) → G2a → STAGE → EXEC-1~8(C6-PRE migration **실행**은 EXEC-3 — build는 a/b1/b2 LAND) → C7.
...
### C6-FLIP Activation Runbook (실행 계획 — 2026-06-20, plan-first 4-lens Workflow `wf_0604216f` + codex 합의, must-confirm 경계)

> A1~B3 + C6-1~C6-9 + C6-5b + C6-quiesce(Q0/Q2a/Q2b/Q4a/Q4b) dormant land 후 atomic-FX flip을 production에서 켜는 실행 계획. **read-only 분석으로 확정**(전 구조 주장 코드 검증). FX scope = bank+investing only(fx_membership.py:17, FX_CUTOVER_ASSETS atomic_cutover_durable.py:30); USDT/KRX 별 트랙. lens 4(wiring/go-gate/C6-PRE/rollback) + synthesis + completeness critic. **production write 위험 = C6 한 점**(§19:323/361)은 불변 — 아래 dormant-landable 단계는 production write 0, EXEC만 write.

**🎯 현재 상태 (2026-06-22 갱신) — ✅✅ CUTOVER EXECUTED & COMPLETE + decouple hotfix 배포·검증**: C6 atomic-FX cutover가 production에서 **실행 완료**. **타임라인**: dormant-landable 전부 LAND(W0/W1 `d7a50f8`/W2 `4909ebd`/lease `3aeac79`/b1 `4f10508`/b2 `29bab06`/docstring `b236435`/W3 `df174ac`/RELEASE) → **DEPLOY+G2a**(`c210e7f`, 2026-06-21) → **EXEC-1~5 첫 시도 성공·EXEC-6 실패**(startup-ordering incident: warmup이 atomic refresh 전 실행 → v2 Redis를 legacy v1로 덮음 → atomic mirror migration_required×30) → **ROLLBACK A**(`4c8536d` 재배포, 서비스 완전 복원) → **startup-ordering FIX**(`a756d31`: refresh→warmup 순서 + is_initialized 가드, codex 3 NO-GO fix) → **orphaned control reset + fixed image 배포** → **EXEC-1~8 재시도 전 단계 성공 + post-EXEC-6 5-signal HARD GATE 5/5 PASS**(2026-06-21 ~21:08 KST, incident 재발 0) = **cutover_state=atomic_ready / publisher_gate_open=true / writer atomic epoch1 / 3 asset ready**. → **후속 decouple hotfix**(`de4fe41`, 2026-06-22: 전역 writer mode가 USDT/KRX Redis writer까지 BLOCK → 테더 탭 freeze → 4 gate 제거 mode-independent, §401-405 Amendment) 배포·검증 완료(FX atomic 무영향 + USDT/테더 탭 복구 확인). **잔존 = C7(post-flip cleanup: trip-wire 제거 / telemetry hardening) only**. ⚠️ 아래 본문은 **실행 전 설계 기록(historical)** — 'must-confirm' / '잔존 EXEC-1~8' / 'EXEC-1부터 live' 표현은 모두 **실행 완료로 superseded**. 상세 incident/timeline: memory [[feedback_atomic_cutover_incident]] + [[project_c6_plan_entry]].

**중심 사실(검증)** ⚠️[**historical — 실행 전 snapshot**; 2026-06-21~22 DEPLOY+G2a+EXEC-1~8 전부 실행 완료 → cutover live(atomic_ready). 아래 'running image IMAGE_MAX=1 / flip 구조적 불가 / 잔존 must-confirm' 절은 실행 전 기준이며 모두 superseded — 현재 상태는 418 헤더]: **코드** IMAGE_MAX=2(C6-FLIP-RELEASE land, atomic_write_control.py:46) — 단 **배포 running 이미지는 여전히 IMAGE_MAX=1**(deploy 별도 operator GO 전)이라 현 running 이미지로 flip **구조적 불가**. begin-atomic capability gate(`IMAGE_MIN≤required_protocol≤IMAGE_MAX` AND `>seed`, activate_atomic_fx.py:761)는 deploy 후 열리나, drain proof(open quiesce session + halt ACK) + seeded/coherent control+cutover state + cas fence로 G2a/EXEC 전까지 여전히 차단. ⚠️ RELEASE는 writer behavior-change-0 — compute_effective_mode가 IMAGE_MAX 미참조(:101-139). + **원래 5 gap + RELEASE 전부 LAND**: ✅ W1 `_do_halt`→cas_open_quiesce_session(`d7a50f8`) / ✅ W2 `_do_begin_atomic`→cas_consume_quiesce_session(`4909ebd`) / ✅ lease brick cas_acquire/renew/release(`3aeac79`) / ✅ prod migration 경로 `--allow-production-migration`(`29bab06`, drain proof+lease+backstop) / ✅ **W3 main()→RealQuiesceBoundary 주입**(`df174ac`, arming — constructor default는 `_FailClosedQuiesceBoundary` 유지 :377, main만 주입) / ✅ **RELEASE IMAGE_MAX 1→2 코드**(behavior-change-0, deploy 별도) — 전부 land·CI green. **잔존 must-confirm**: DEPLOY(IMAGE_MAX=2 image, G2a campaign 동반) → G2a → STAGE → EXEC-1~8 → C7.

**재분류(pivot)** ⚠️[**historical — 실행 전**; 아래 '잔존 must-confirm: DEPLOY·G2a·STAGE·모든 EXEC·C7'은 2026-06-21~22 전부 실행 완료, 잔존=C7 only — 418 헤더 참조]: "남은 건 전부 must-confirm"은 부정확 — **planning-only prerequisite**(코드/prod 변경 0): ✅ W0(session-id 계약 LOCK, codex 019ee2ee). **dormant-landable ✅ 전부 LAND (2026-06-21, behavior-change-0·CI green)**: W1(`d7a50f8`)·W2(`4909ebd`)·C6-PRE-build-a lease(`3aeac79`)·b1 drain-proof helper(`4f10508`)·b2 migration gate(`29bab06`)·docstring(`b236435`) **+ W3 arming(`df174ac`) + RELEASE(IMAGE_MAX 1→2 코드, behavior-change-0)**. **잔존 must-confirm**: DEPLOY(IMAGE_MAX=2 image)·G2a·STAGE·모든 EXEC·C7.

**ordered sequence**:

- **[planning-only prerequisite] W0 ✅ LOCKED** (2026-06-20, codex thread 019ee2ee GO) — session-id(activation lineage id) 계약, 코드/prod 변경 0:
  1. **halt가 lineage id 확립**: `--session-id` 필수를 halt phase로 이동. `_do_halt`가 cas_request_halt APPLIED 직후 **같은 tx**에서 `cas_open_quiesce_session(session_id=id, halt_mode_generation=writer_generation+1, halt_committed_at=durable halt barrier ts[utcnow, insert 직전 최대한 늦게 — DB commit ts 아닌 halt barrier])` → 1 commit. gen pin=writer_generation+1(staged halt UPDATE가 populate_existing 재read에 보임).
  2. **begin-atomic은 DB-authoritative discover**: `find_open_quiesce_session(db)`로 open session 찾아 그 id를 `cas_begin_cutover(new_session=discovered)` + `cas_consume_quiesce_session(session_id=discovered)` 양쪽에 사용(**single shared id**), 한 begin tx(cas_activate_atomic 동반). open 없으면 fail-closed(drain 미완).
  3. **begin-atomic `--session-id`는 optional cross-check**(codex 판단): 주면 discovered와 != 시 fail-closed(operator-error 검출), 없으면 discovered 사용(DB 권위 불변).
  4. **activation halt만 quiesce session open**: incident-halt(cas_request_incident_halt, epoch>=1)는 cas_open 미호출 — cas_open activation_epoch==0 fence가 구조 backstop.
  5. W1이 test_halt_applied / test_halt_expected_generation_match_applied 갱신(activation halt에 `--session-id` 필수).
- **[dormant-landable ✅ 전부 LAND 2026-06-21]** **W1**(`d7a50f8` _do_halt+cas_open_quiesce_session same tx) → **W2**(`4909ebd` _do_begin_atomic+find_open_quiesce_session+cas_consume + cutover bootstrap=discovered + dry-run parity) + **C6-PRE-build-a**(`3aeac79` lease brick, **app-clock Option A** 채택 — DB-clock 대신 cross-dialect testability, codex 019ee30e) → **b1**(`4f10508` 공유 `confirm_quiesce_drained` drain-proof helper + RealQuiesceBoundary 위임) → **b2**(`29bab06` `--allow-production-migration`: **drain proof[confirm_quiesce_drained] ∧ lease ∧ _check_apply_target backstop[control/lease fresh re-read, env-mismatch+undetermined-host block]** 충족 시만 prod; codex 2 prod-safety blocker fix) + **docstring**(`b236435` critic#7 "atomic=write 0"→실제 v2 write). ⚠️ **b2 설계 정정**: 원안의 "halt-mode 확인[requested_mode==HALT]만"은 codex NO-GO(**HALT row != quiesced writers** — stale legacy-cached writer가 v1 덮음) → **drain proof(b1 공유 helper) 요구로 재설계**됨.
- **[W3 ✅ LAND `df174ac`, arming]** main()이 `AtomicFxActivator(db, quiesce_boundary=RealQuiesceBoundary())` 주입 + trip-wire **test_main_injects_real_boundary**(`test_main_does_not_inject_real_boundary`에서 invert; **AST 구조 검사** — getsource substring은 주석 false-pass라 codex 지적 후 AST로); legacy steady-state엔 inert(prod behavior-change-0). → **[✅ RELEASE 코드 LAND, deploy 별도 GO] C6-FLIP-RELEASE**(IMAGE_MAX→2 atomic_write_control.py:46; behavior-change-0 land, **DEPLOY 시 production-visible** — begin-atomic 구조적 가능해짐)
- **[must-confirm, prod 준비]** **G2a**(prod RDS 3 migrate: write_control/cutover/quiesce, idempotent net-new, create_all 제외 database.py:49-54) → **STAGE**(전 시퀀스 staging 리허설 + lease lifecycle + rehearse_fx_cutover.py + service-behavior-0 payload diff; staging도 IMAGE_MAX=2)
- **[must-confirm, 비가역]** **EXEC-1** RDS snapshot+halt(legacy→halt, FX publish gate close, baseline WS 무영향 :650) → **EXEC-2** recreate-drain(flip-release image, fresh process가 durable halt ACK record_app_ack_if_quiescing main.py:544-545) → **EXEC-3** C6-PRE migration(halt window 안, Option B no-race, `--scope all`=FX-all[latest:source:* selector 부재]) → **EXEC-4** verify(dry-run all already_current + verify-only 3 asset; conflict/invalid_schema→STOP) → **EXEC-5 begin-atomic(★THE FLIP: halt→atomic + cutover idle→running 1 tx, activation_epoch=1 one-way door)** → **EXEC-6** recreate-atomic(writers atomic + cache refresh §9:90) → **EXEC-7** verify(running→verified) → **EXEC-8** finalize(asset ready×3 먼저 + verified→completed 마지막 1 tx, **★FX publish gate OPEN**)
- **C7** post-flip cleanup(별 트랙). **✅ C7-a telemetry hardening LAND**(2026-06-22): writer-side atomic outcome counter(구 write-only crud `_atomic_write_outcome_counts`)를 read accessor `get_atomic_write_outcome_counts()` + `GET /admin/api/atomic-write-outcomes`로 surface(aggregate/per_source by_state + critical[conflict+failed_structural=§17 corruption=G3] + health(ok|critical)/g3_ok, never-crash, process-local + reset route 없음). **G3 관측 gap[crud write-only] 해소**. trip-wire 제거 = 거의 no-op(Explore 평가 — load-bearing invariant + 정당한 dormant scaffolding[publisher/coordinator측, fanout 트랙용]은 KEEP, misleading 1건만 caveat 후보). "Legacy FX hook 제거"는 PR D/fanout 트랙(별도 — 지금 제거 시 mirror trigger 미발사로 publish recovery 깨짐, test-locked).

**where dormant ends**: ✅ W1/W2/C6-PRE-a(lease)/b1/b2/docstring LAND dormant + **W3 arming LAND(`df174ac`)** + **RELEASE IMAGE_MAX 1→2 코드 LAND**(2026-06-21, CI green, behavior-change-0). W3 = inflection(arming); RELEASE = capability 코드 land(deploy 전엔 prod-inert). **DEPLOY(IMAGE_MAX=2 image)가 다음 단계**(running image still 1 — deploy 시 begin-atomic 구조적 가능). EXEC-1부터 live production. **→ ✅ 2026-06-21~22 전부 실행 완료(historical): DEPLOY+G2a+EXEC-1~8 → cutover live(atomic_ready) + decouple hotfix 배포(§401-405 Amendment). 잔존=C7 only. 상세는 위 🎯 현재 상태 블록.**

**go-gate G1~G7**(HARD-NO-GO vs operational):
...
- **G3** control-plane coherent/no corruption — HARD (resolve_resume_action≠FAIL_CLOSED, derive_cutover_state≠corrupt-HALT_BLOCKED; **관측 gap [✅ C7-a 부분 해소]**: writer-side conflict/structural counter는 `/admin/api/atomic-write-outcomes`로 surface(crud write-only 해소); coordinator-side persisted counter(atomic-cutover-status future stub `available:false`)는 coordinator dormant라 유지)
...
- **G6** IMAGE_MAX≥2 + 새 image deploy *before* begin-atomic — HARD (chicken-egg: old image MAX=1이면 gate 불만족 → 새 image recreate 후 begin-atomic; row가 protocol=2면 old-image rollback은 자기 preflight 실패=intended §6)
- **G7** quiesce drain confirmed — HARD (RealQuiesceBoundary.confirm_quiesced() True = open session ∧ FRESH control[HALT/exact gen pin/epoch0] ∧ qualifying ACK[cond2/3/4]; **W1+W3 선행**)

ordering: G1+G2a(pre-window) → G2b(EXEC-3/4) → G3(continuous) → G5+G6(EXEC-2 recreate + EXEC-5) → G7(EXEC-5). G4=STAGE prerequisite.

**rollback/crash-resume**:

- rollback = **atomic→halt only**(`cas_request_incident_halt` atomic_write_durable.py:122-158, fence requested_mode==ATOMIC, epoch/activated_at 보존; `--phase incident-halt`, snapshot 불요 :629-632). atomic→legacy **구조적 금지**(어떤 brick도 requested_mode=legacy 미작성 + compute_effective_mode epoch>0→HALT 강제 :129-131). halt이 안전 terminal(derive_cutover_state HALT→HALT_BLOCKED, stale cutover 무관). halt은 FX publish만 close, baseline WS 독립(:650). ⚠️ halt은 writer 축 GLOBAL이나 mode-dependent writer는 FX(bank/investing)+mirror뿐 — USDT/KRX Redis-latest는 decoupled(mode-independent, §401-405 Amendment, 2026-06-22)라 halt에도 계속 write(freeze 없음, disjoint keyspace). **post-completed re-activation 미구현** — completed에서 in-place 복구는 incident-halt뿐.
- crash-resume = `--phase auto`, resolve_resume_action(:209-251) 양축 fresh DB read: LEGACY+idle→HALT / HALT+idle→BEGIN_ATOMIC / ATOMIC+running→VERIFY / ATOMIC+verified→FINALIZE / ATOMIC+completed+ready→NOOP / torn→FAIL_CLOSED. 각 phase own-tx caller-commit이라 commit 전 crash는 축 불변. EXEC-1~5 사이 crash(halt+idle+open session) → BEGIN_ATOMIC + boundary가 ACK 재검증(open quiesce session은 halt 축이라 cutover resume 무교란 — 명시 test 후보).
- SEV: writer halt=**SEV-1**(FX publish stop, WS 무영향) ⊥ readiness-drop=**SEV-2**(D-recoverable re-migrate) ⊥ revision-lag=**no-action**(bounded-eventual).

**completeness critic 핵심(+ 보완 A~D)**:

1. **recreate-drain 최약점(보완 D)**: boundary-True ≠ all-writers-drained — in-process counter 폐기(C6-quiesce pivot)라 cross-process "all drained" 관측 **본질적 불가**. Selenium child 1개 생존+main ACK면 boundary 통과 가능. 코드 완전 해결 불가 → **runbook operational 체크**: EXEC-2에서 old container 완전 down(Selenium subprocess/Chrome child 포함) 확인 후 new image up.
2. **lease clock/TTL(critic #6)**: **DB-clock 권장**(skew→premature takeover/never-expire 회피), TTL>migration 최악(FX key ~수십개라 generous one-shot TTL이면 renewer thread 불요). C6-PRE-build-a 명시 결정.
3. **stale docstring(보완 B/critic #7)**: ✅ activate_atomic_fx.py gate-3 docstring(:19-27) C6-FLIP-RELEASE에서 정정 — IMAGE_MAX=2 반영 + critic#7 invariant('atomic writer는 실 v2 write, flip은 항상 real write 가정') 보존, blocker를 capability→drain proof+seeded state+cas로 갱신.
4. **atomic reader latest:source:* v2 assert?(보완 C/critic #5)**: EXEC-6 대기 말고 **dormant build 중 read-only 선검증**(allowlist-skip 확인 — usdt_topic_payload.py:281/:327 DB-fallback 경로).
5. **worker==1 비강제(critic #1)**: safe-by-CAS이지 safe-by-lock 아님 → runbook "한 operator, 한 shell, no parallel phase".
6. **USDT/KRX decoupled (구 critic #4 global freeze — 2026-06-22 hotfix로 해소)**: usdt/krx Redis writer는 mode-independent(§401-405 Amendment)라 flip/halt에 freeze 안 됨. latest:source:*는 FX atomic과 disjoint keyspace(loader/mirror 안 읽음). `--ack-global-writer-mode-scope`는 여전히 동반하나 의미가 'mirror만 mode-dependent'로 축소 — USDT/KRX 영향 없음.
7. **in-flight straddling halt commit(critic #10)**: low-churn window 권장. (구 'global freeze가 KRX close write 건드림'은 2026-06-22 decouple로 해소 — KRX close finalizer는 이제 mode-independent라 halt/atomic에도 정상 write. KRX close window[~15:35-15:50 KST CF, ~05:50-06:05 CM] 회피는 telemetry 깔끔함 목적의 optional.)

**key anchors**: W1 activate_atomic_fx.py:485(_do_halt) / W2 :521(_do_begin_atomic) / W3 :907(main construct, :377 default) + tests/test_atomic_quiesce_boundary.py:164-181(W3 inject tripwire) / quiesce bricks atomic_quiesce_durable.py:71/:115/:179/:197 / ACK writer atomic_quiesce_startup.py:44-95 wired main.py:544-545 / capability gate activate_atomic_fx.py:753-770(:761 check) + atomic_write_control.py:42-47 / lease columns models.py:348-349,:362-363 + ✅ brick cas_acquire/renew/release atomic_cutover_durable.py(`3aeac79`) / migration hard-block(no-flag) atomic_migration.py:483-509 + ✅ b2 override `_check_apply_target(authorized)`/`_revalidate_production_authorization`(`29bab06`) / confirm_quiesce_drained atomic_quiesce_durable.py(`4f10508`, RealQuiesceBoundary+migration 공유) / C6-9a endpoint main.py:1449-1458.
