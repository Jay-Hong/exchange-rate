...
- **🔜 S7** (parity 기준, 향후): **inline dual-compute** — `process_rate_alerts`의 legacy pre-mutation 지점에서 새 evaluator condition 로직을 동기 적용해 legacy `triggered_items`와 비교(race 0, cache 0). post-legacy async shadow로는 구조적 불가. cutover(open decision 7) 판단 근거.
