...
**✅ live prod 실측 완료 (2026-06-24 16:04 KST, SSH read-only: .env grep + container-internal admin API)**:
- `BANK_INVESTING_TOPIC_TRIGGER_MODE` = **direct_coalesced** (.env + 런타임 `trigger_flush_direct` usd 51652/jpy 26385/eur 19817 ≫ 0 = effective 확증; `trigger_flush_dual_shadow` 3591/1914/1493은 dual_shadow→direct 롤아웃 history cumulative). → **backward-flip 금지 anchor 확정**.
- writer mode = **atomic** (effective_mode=atomic, enforced_action=atomic, writer_enforced=true, activation_epoch=1, 2026-06-21 21:08~). → live fanout = atomic 분기(`applied_for_trigger` SET-only), C6 cutover 안정.
- `FX_TOPIC_ENABLED`=true / `TOPIC_DISPATCHER_ENABLED`=true / `BANK_INVESTING_TOPIC_TRIGGER_COALESCE_MS` 미설정→default 500ms (런타임 `trigger_last_window_ms`≈500.5 확인).
- **fx:* subscriber_count = 0 (3 topic 전부)** — **live 단말이 fx:* topic 미구독**(앱은 legacy WS `rates` 사용 중). C1 direct + legacy hook 둘 다 0명에게 publish(`trigger_last_result=publish_zero`, hook `skipped_no_subscribers`≈hook_called). → **C2/C3/hook(step 7/8)은 전부 consumer-0 인프라 작업** = decision 1(no consumer)/2(hook recovery 공백도 client migration 전까지 이론적) resolution 강화. 이상 징후 0(trigger_error 0 / error 0).
- ⚠️ **신규 sequencing 함의**: topic fanout(C1/C2/C3/hook)은 fx:* 구독자 0이라 **client topic-migration이 선행 안 되면 전부 pre-consumer**. mirror-retirement(A)은 반대로 live-consumer인 legacy WS broadcast 경로의 DB-fallback/freshness 트랙 → 우선순위 비교 시 "어느 경로가 live consumer를 갖나"가 추가 축.
