...
**날짜**: 2026-08-28
**상태**: Accepted — 코드 `8cf9da3` push, 2026-08-28 운영 배포·야간 관측 완료
...
## ADR-044: 우리은행 야간 수집 창을 06:00 정책으로 이동 — 관측과 정책의 분리

**상태**: Accepted (2026-09-10)
**범위**: 우리은행 BREAK1 창 + BREAK2 마무리 조회. 다른 은행·모드 경계·OUT 주말 정책은 불변.

### 맥락

ADR-042는 우리은행 고시 종료를 **익일 05:00경**으로 잡고 수집을 `05:04:53`에 끝냈다.
그 전제가 반증됐다 — 사용자가 은행 페이지에서 **2026-09-05(토) 05:55:56 고시**를 직접
확인했고, 그 시각은 구 창 종료의 51분 뒤였다.

토요일이라 BREAK2(woori 미등록)·OUT(제외)이 연달아 이어져 복구 기회가 0이었다.

관측된 것과 추론을 나눈다. **관측**: 사용자가 은행 페이지에서 05:55:56 고시를 확인했다.
**운영 DB**: `woori usd-krw` 가 `09-05 03:55:44` 다음 곧바로 `09-07 08:00:14` 로, 그 사이
약 52시간 동안 행이 0건이고, 08:00:14 에 3통화가 함께 저장됐다.
⚠️ DB 기록만으로 **놓친 고시의 횟수나 동일성을 확정할 수 없다** — 3통화 동시 저장은
change-only 저장의 결과이지 "한 회차가 통째로 밀렸다" 는 증명이 아니다. 말할 수 있는 것은
바뀐 값이 월요일에야 저장됐다는 것뿐이다.

⚠️ 최근 90일 woori 의 `05:05~07:59` 행은 0건이고, 같은 창에 다른 은행은 활발하다
(30일: hana 2051 · kb 685 · bs 384 · citi 193 · ibk 190). 다만 **행 0건이 조회 0회를
증명하지는 않는다** — change-only 저장이라 값이 같으면 행이 안 생긴다.

코드에서 확인한 사실은 구간에 따라 다르다. `05:05~05:59` 에는 BREAK1 의 woori job 이
**등록돼 있었지만 그 시간대에 발화하지 않았다**(구 트리거의 마지막이 05:04:53). `06:00`
이후 BREAK2 에는 woori job 자체가 **등록되지 않았다**. 둘을 뭉쳐 "미등록" 이라고 쓰지 않는다.

### 결정

1. BREAK1 을 `hour='19-23,0-5'` `:14/:44` 로 단순화한다. 마지막 발화 **05:59:44**, 총 1,320회.
2. BREAK2 에 마무리 조회를 둔다 — `06:00~06:03 :14/:44` + `06:04:53`, `day_of_week='tue-sat'`,
   9회. 야간 세션이 평일 19:00 → 익일 06:00 이라 화~토 아침에만 끝난다.
3. 합계 1,329회로 기존 1,209회 대비 **+120**. 이는 **예약 횟수**이지 실행·성공·포착 보장이 아니다.

### ⛔ 마무리 조회는 같은 `task_woori` ID 여야 한다

06:00 전환을 걸친 실행이 마무리 발화와 겹치면 Chrome 이 2개 뜬다. 이를 막는 것은 **시간
간격이 아니라 job ID** 다. APScheduler executor 는 `_instances[job.id]` 카운터로
`max_instances` 를 강제하고, 그 카운터는 `remove_job`/`add_job` 으로 사라지지 않는다.
같은 ID 면 05:59:44 실행이 살아 있는 동안 마무리 제출이 `MaxInstancesReachedError` 로
차단되고, 다른 ID 면 별도 카운터라 동시 실행된다(양성 대조 시험으로 잠금).

⛔ 간격 확보는 해결책이 아니다 — 실행시간 상한이 확정되지 않았고, 그 새벽에는
`is_mibank_rate_reliable()` 가 MIBANK 를 막아 기존에 상정한 최악 경로도 그대로 적용되지 않는다.
⚠️ 이것은 크롤러 **호출** 중복 방지다. 타임아웃 뒤 남는 Chrome 은 별개 문제로 남는다.

### 주장하지 않는 것

- **05:55:56 은 "05:00 이후에도 고시한다" 는 관측이지 "정확히 06:00 에 끝난다" 는 증거가
  아니다.** 06:00 은 우리가 정한 **수집 종료 정책**이다.
- 다른 은행의 DB 저장시각으로 우리은행 공식 고시시각을 확정하지 않는다. 그것은 그 시간대에
  수집이 일어난다는 증거일 뿐이다.
- **토 06:04:53 ~ 월 08:00 은 미관측 구간**이며, 그 구간에 추가 고시가 없음을 보장하지 않는다.
  OUT 주말 수집 추가는 이번 범위 밖이다(관측 한 건으로 주말 정책까지 바꾸지 않는다).
- 과거 누락 데이터의 소급 수정은 하지 않는다.

### 완료 기준

새벽 연장 구간에서 **마지막 확인 기회가 실제로 완료됐는지**와 공식 응답→DB·Redis/API
정합성을 함께 본다. 지연으로 끝까지 실행되지 않았다면 "확인 완료" 로 보고하지 않는다.
값이 같아 새 DB 행이 없는 것은 정상이다.

### 2026-09-18 운영 관측 — 예정 9개 슬롯 중 시작 7회 확인, 모드 전환 지연으로 첫 2개 슬롯 미실행

⛔ **결정 상태는 Accepted 유지. 다만 위 완료 기준은 미충족이다.**

**관측 환경**: 코드 `721a3237900961cf59e9a50a98ce20357bc56755`, 이미지
`sha256:eec4a8025dcb99531598462d6e8cbcf19a70bdb7e99ccff87f8cd90e637b9b03`.
근거는 보존 run `22d9e7b28ada44be90ba97b5d105b233` 의 `stdout.raw`
(sha256 `626d512cb0a16901bb5b76d0912e7f8aed339773b3ecaecce784cb0b53879733`, 7,236,365 B,
요청 창 `2026-09-17T20:13:01Z ~ 21:23:01Z` 로 우리 창을 덮음, `phase=finished`·`ok=true`·
`cursor_committed=true`). 집계기 보존본 검증 통과.

**핵심 로그 시각** (UTC / KST):
```
21:00:02.170Z 06:00:02  WARNING  Run time of job "control_job (trigger: cron[second='1'],
                                 next run at: 2026-09-18 06:01:01 KST)" was missed by 0:00:01.170653
21:01:01.008Z 06:01:01  INFO     Removed job task_woori
21:01:01.021Z 06:01:01  INFO     Added job "request_wrapper_woori" to job store "default"
21:01:01.022Z 06:01:01  INFO     === BREAK2 모드로 전환됨 ===
```

| 슬롯(KST) | 시작 | writer 진입(배너) | 회차 귀속 commit 로그 | 종료 상태 |
| --- | --- | --- | --- | --- |
| 06:00:14 · 06:00:44 | ❌ | — | — | **실행 기회 없음** |
| 06:01:14 ~ 06:03:44 (6회) | ✅ | ✅ | 미관측 | **미확인** |
| 06:04:53 | ✅ | ✅ (+196ms) | 미관측 | **미확인** |

⛔ **"시작·writer 진입 7건" 은 완료 7건도, 수집 성공률도 아니다.**

**미실행의 정확한 기전**: `task_woori` 는 BREAK1 에도 **이미 존재**했다. 없었던 것은
**BREAK2 마무리 트리거**다 — BREAK1 트리거(`hour='19-23,0-5'`)에는 06:00 슬롯이 없고,
마무리 트리거는 06:01:01 의 remove/add 로 비로소 설치됐다. 따라서 이 두 슬롯은
woori job 자신의 misfire 나 `max_instances` 차단과 **구분해야 한다**.

**확인된 직접 경로**: `task_woori` 는 `misfire_grace_time=30` 을 명시한다(관측 코드 `721a323` 의
`scheduler.py` 982행, BREAK1 등록부).
...
**같은 날 08:00 BREAK2→IN 전환도 한 분 늦었다.** `08:00:02.436` 에 control_job misfire(1.44초)
경고가 나고, 이어 `08:01:01.022` 에 IN 전환 로그가 찍혔다.

**정각 misfire 분포 — 보존 원문에서 다시 계산**: 구 컨테이너(`23d43b27e310`, 기동
`2026-09-17T16:57:48Z`)의 Investing 관측 보존 원문 13개 run(창 `2026-09-17T16:48:45Z ~
2026-09-18T04:26:05Z`, 창끼리 10분 겹침은 `(timestamp, message)` 로 중복 제거)에서 "was missed" 19건:

| job | 건수 | 시각(KST) · 지연 |
| --- | --- | --- |
| `control_job` | 5 | 04:00 (1.77s) · 06:00 (1.17s) · 08:00 (1.44s) · 10:00 (1.67s) · 11:00 (1.41s) |
| `report_queue_status` | 13 | 02~12시 매 정각(1.02~2.43s) + 10:33 · 10:40 |
| `record_monitoring_stats` | 1 | 10:33 (1.02s) |

관측 창 안의 정각 12회(02~13시) 중 11회에 misfire 가 있었고 13:00 에는 없었다. 전환 로그는 01:58:00
(기동)·06:01:01·08:01:01 세 건이다.
⚠️ 이것은 보존 원문에서 확인한 **발생 시각과 지연의 분포**다. 정각에 단일 원인이 전체 실행을 막았다는
증거가 아니다 — 예정 시각·실제 시작·실행 시간·executor 대기는 이 로그로 구분되지 않는다. 원인 조사는
별도로 한다.

원문은 서버 `~/logs/investing-observe/runs/<run_id>/stdout.raw` 에 있다(보존 디렉터리 0700, 원문 파일 0600). 중복 제거 전 파일 해시:

```text
f1c42aa76d6a43c8a45649c70fdb36fb  fc086f097e5ad69c7c5231f7bcdfc31f729cc9a34cfa8d2c133c8071aac9f416
682aad77a66245ac879630081b68f482  c72edf97b59d14f29c4e515e61f5ee887b91587fb596f007f00f049d13e52044
4675903be8ce4df69a0b12cd9852a9b9  693f6861abd0eca72f34e1d665c7c48376ccb749f8c553df8511d7200fc60a9e
22d9e7b28ada44be90ba97b5d105b233  626d512cb0a16901bb5b76d0912e7f8aed339773b3ecaecce784cb0b53879733
7348370f50a84481b9295de643f7ded9  530cb5d585bae8a66c02723dc3ad1fa3c8fa1ddbfdbc4deb52090d32a3b02b97
6a056d5cc4ac4af399d3baacb7871e3f  4158ac5a36e0472e3eb4586ab720cfd8415da5f45920c2b73085ce772c2762fe
722323afb0c242c0b8a61e4574e1844f  23e42a1664f6fb1e80e7a4144753be120f71c9e36a8bc3bf34fcbc1b8bd5245f
f524284160204917b918a34b1f73a817  f762917482bdb58617287564dbf5ea7ae4771784c4f57656b391670baba7855c
20634a5cb5aa4d0e8f946f50af0213b2  b28a61b397ba8d4064b8ece6019b243e8cc26fdcbb5ebdf3772157582926de30
f7208ee35c344516bf47a06e33df44f7  867b0b012f3e5436370d93fb936a43536b687d47795f1632a1e85333a610eb7f
72544893b087495d9120d8c0a92f7a11  bfba05e536d56f6bec2a088640edb52a2dfa36610e636d67960de26cc8219842
10509f86253d4432a9baf45a49496b54  410df39d9da1c5e647230ade53846b8482336e8a1353c5c0a0cf37aa2611b076
ed51082a5783410cae101f7eb15a3722  4f76317342f6a86cf5fab026f05d9ab560ecfd5554b7d039107a3e4f2b6eeadf
```

**사후 정합성** (조회 시각 `2026-09-17T22:08:18Z` / `07:08:18 KST`):
DB↔Redis 는 **세 통화** 일치(usd 1382.0 · jpy 886.75 · eur 1587.23), API 는 **USD 만** 확인.
사후 DB 조회에서 **06:00 KST 이후 woori 신규 행 0건**.
⚠️ 이 일치는 조회 시각의 정합성이지 06:04:53 직후 전달 완료의 증거가 아니다.
⚠️ 04:51 KST 의 EUR 행은 그 시각 DB 쓰기가 있었다는 증거이며, 현재 컨테이너의 특정 writer
호출에 귀속하려면 추가 연결 근거가 필요하다.

**왜 미확인이 남는가**: 회차 정상 완료(`✅ [woori] 완료`)와 값 불변(`📼 [유지]`), 쓰기모드
skip 기록이 **모두 DEBUG** 이고 운영은 `LOG_LEVEL=INFO` 다. 또 Selenium 폴백은
`capture_output=True` 라 자식 로그가 부모 Docker 로그에 오지 않는다(자식의 파일 로그 경로는 따로 있다). 즉 **값이 바뀌지 않는 정상 회차에서는
현재 관측면으로 응답→저장 연결을 지을 수 없다.** 관측 공백 전체는
[SOURCE_HEALTH_PLAN.md §2.2](SOURCE_HEALTH_PLAN.md#22-운영-로그에서-보이지-않는-사실)에 모았다.

**열린 항목 2건** (보고 계측과 분리해 다룬다):
1. **`control_job` 지연 허용 정책** — 기본 1초가 의도인지. 검토 조건: grace 를 늘렸을 때의
   복구 범위와 **더 긴 지연에서의 동작**. 이 경계는 19:00·08:00·토 07:00·월 06:00 에도 있다.
2. **경계 시각의 job 등록 정책** — 첫 슬롯 전 등록 실패, 늦은 전환, 기존 실행과의 중복 방지.
   ⚠️ **grace 확대만으로 이미 지나간 미등록 슬롯이 복원되지는 않는다.**

### 2026-09-18 후속 — 열린 항목 1 구현·운영 반영: 지연 허용 정책 명시와 전환 기록

**운영 반영(2026-09-18)**: 코드 `13bc1a3`, 이미지
`sha256:683de70ae4d24ba222eee9f058b0b920a130c80ff4909381ff6e868cc36d234c`.
switch 명령 종료 13:26:12 KST(새 컨테이너 StartedAt `2026-09-18T04:26:09.985Z`), startup 전환 기록 시작
13:26:20.564 KST, promote 13:29:33 KST. ⚠️ promote 는 합의한 13:3x 창보다 먼저 실행됐다(제외 분
게이트는 통과). 새 컨테이너의 시작 기록 검사가 통과했다 — `startup` 시작·종료 각 1건이 같은
`switch_id` 로 짝지어졌고, IN · `scheduler_running=false` · add 11건 = 종료 시 `task_*` 11개 · 실패 0,
scheduler 계열 오류·예외 0.

⛔ 열린 항목 1 은 **경계 전환(19:00·06:00) 기록으로 확인하기 전까지 닫지 않는다.** 열린 항목 2 는
그대로 열려 있다. 경계 관측이 통과해도 지나간 슬롯 복원·정상 완료·응답→저장 연결까지 해결됐다는
뜻은 아니다 — 열린 항목 1 과 ADR-044 전체 완료는 따로 판정한다.

**A — 지연 허용 정책**: `control_job` 등록부가 `app/scheduler.py` 의 `CONTROL_JOB_OPTIONS`
(`misfire_grace_time=30` · `coalesce=True` · `max_instances=1`)를 쓴다. 30초는 **늦은 모드 복구를
허용하는 정책**이지 첫 슬롯 보장값이 아니다.

운영의 `AsyncIOScheduler` 는 동기 함수인 `control_job` 을 기본 스레드 풀에서 APScheduler 의
`executors.base.run_job` 으로 돌리고, misfire 판정도 그 안에서 한다. `tests/test_control_job_transition.py`
는 판정을 흉내 내지 않고 그 `run_job` 을 시각만 고정해 직접 부른다. 전환 뒤 첫 발화는 새로 등록된
우리 트리거가 계산한 **다음 예정 시각**이다.

| 06:00:01 실행의 지연 | 판정 | 전환 뒤 우리 트리거의 첫 예정 시각 |
| --- | --- | --- |
| 1.17초(이번 관측) | grace 1초 → 건너뜀(운영과 같음) / 30초 → 실행 | 06:00:14 |
| 15초(06:00:16) | 실행 | 06:00:44 — **06:00:14 는 이미 지났다** |
| 31초 | 건너뜀 | 다음 발화 06:01:01 이 전환 → 06:01:14 |

⚠️ 시험 범위는 misfire 판정과 트리거 등록까지다. `switch_jobs()` 자체의 소요 시간, 실제 크롤러
실행과 큐 처리는 시험하지 않았다. 30초를 넘는 지연·연속 misfire·전환 실패에서는 다음에 성공한
발화까지 구 모드가 이어진다. **1.17초 지연의 근본 원인은 여전히 모른다.**

**B — 전환 기록**: `switch_jobs()` 호출마다 INFO JSON 이벤트 둘을 같은 `switch_id` 로 남긴다.
시작(`mode_switch_started`)은 본문 **전에** 내보내, 본문이 멈추거나 끝나지 못해도 그 호출이
시작됐다는 기록이 먼저 로그에 나가 있게 한다. 종료(`mode_switch_finished`)에 결과를 적는다.

| 필드 | 이벤트 | 뜻 |
| --- | --- | --- |
| `switch_id` | 둘 다 | 호출별 식별자 |
| `cause` | 둘 다 | `scheduled_control` / `startup` / `admin_toggle` |
| `requested_mode` · `observed_mode_at_start` | 둘 다 | 요청 모드와 호출 시작 시 `current_mode` |
| `scheduler_running` | 둘 다 | 시작 경로는 `scheduler.start()` 전이라 `false` — 이때의 등록은 pending 이다 |
| `policy_boundary` | 둘 다 | 정기 제어에서만. 시작 시에는 과거 모드 지속을 추정하지 않는다 |
| `started_at` | 둘 다 | 래퍼 진입 시각 |
| `observed_jobs_before` · `observed_jobs_after` | 시작 · 종료 | 전역 job ID 스냅샷. **보조 자료** — 겹친 다른 호출의 변경이 섞일 수 있다 |
| `operations` | 종료 | 본문이 실제로 부른 제거·등록(job ID · 트리거 · 실패 시 예외 종류 · 시각). 시각은 스케줄러 호출이 반환된 뒤의 기록 시각이다 |
| `operations_complete` | 종료 | 작업 한 건의 기록이라도 실패하면 `false` |
| `ended_at` · `duration_ms` | 종료 | 호출 구간(시작 스냅샷 시간 포함) |
| `delay_from_boundary_s` | 종료 | 정책 경계 → 래퍼 진입. `control_job` 시작 지연도, 등록 완료 지연도 아니다 |
| `outcome` · `exception_type` | 종료 | `completed` / `raised`. 중간 예외를 정상 완료로 적지 않는다 |

`operations` 는 본문의 제거·등록 호출이 기록 헬퍼를 거치며 **호출한 스레드의** 기록에 적는다.
전역 스냅샷의 차이로 대신하지 않는 이유는 둘이다 — 같은 ID·같은 트리거로 다시 넣으면 차이가 0 이고,
겹친 다른 호출의 변경이 섞인다. 두 호출을 겹쳐 돌리는 시험에서 각자의 `operations` 는 자기 작업만
담고, 보조 스냅샷에는 다른 호출의 job 이 들어가는 것을 함께 확인했다.

⛔ 이 기록은 **스케줄 등록 사실**이다. 실제 발화·큐 실행의 증거가 아니다.
⛔ 종료 이벤트가 없으면 그 호출의 완료·겹침 여부는 **미확정**이다("경합 없음" 이 아니다).
⚠️ 기록 실패는 전환을 막지 않고 본문 예외를 삼키지도 않는다(시험으로 확인).
⚠️ 관리자 토글과 정기 전환은 **여전히 직렬화되지 않는다(미해결 — 이 절의 코드 기준. 아래 2026-09-19 후속에서 직렬화)**. 기록의 결속만 호출별로 나눴다 —
전역 job 상태는 여전히 겹친 호출이 함께 바꾼다. 겹침은 시작·종료 구간으로 사후 분석한다.
⚠️ 운영 로그는 직렬화기의 이스케이프 방식에 따라 단순 문자열 검색이 조용히 누락될 수 있다 —
이 이벤트는 JSON 으로 파싱해 센다.

**후속으로 미룬 것**: 경계부터의 정확한 슬롯 열거와 실행 여부 판정 · 토글 직렬화(아래 2026-09-19 후속) ·
1.17초 지연 원인 · 경계 슬롯을 전환에 의존하지 않게 하는 구조적 해결.

**경계 전환 관측 (2026-09-18 ~ 09-21, 다섯 경계)**: 앞 네 경계는 컨테이너 `97abe1128e46…`·이미지
`683de70a…`(13bc1a3), 월 08:00 은 R1 배포 뒤 `c152ff8dcfe4…`·`d18b5d68…`(f94d04a)다. 둘 다 재시작 0.
관측 스크립트가 경계 뒤 3~7분에 서버에서 Docker 로그를 읽어 `mode_switch_*` 이벤트를 JSON 으로 다시 파싱했다
(금 19:00·토 06:00·토 07:00 은 검사기 SHA-256 `4bf19484…`, 월 06:00·월 08:00 은 창이 닫히기 전에 읽지
않도록 가드를 넣은 `8c1ca336…`. ⚠️ 앞 세 경계의 결과 원문 파일은 세션 작업 폴더와 함께 **사라졌다** —
수치는 Investing 관측 cron 아카이브(`~/logs/investing-observe/runs/*/stdout.raw`)에서 다시 뽑아 확인했고,
검사기 출력이 남아 있는 월 06:00 을 같은 스크립트로 재도출해 글자 단위로 일치함을 **양성 대조**로 먼저 확인했다.
재도출은 수치뿐 아니라 검사기 판정 항목 — 시작·종료 한 쌍, 창 안 전환 1건, `was missed`·ERROR·traceback 0건,
등록·제거 집합이 전환 전후 `task_*` 집합과 일치 — 까지 덮는다.)

| 경계 | 원인 | 요청 모드 ← 시작 모드 | `started_at` | `delay_from_boundary_s` | `duration_ms` | 제거·등록·실패 | 경고·오류 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 금 19:00 | `scheduled_control` | BREAK1 ← IN | 19:00:01.008 | 1.008 | 61 | 11 · 10 · 0 | misfire 0 · ERROR 0 · traceback 0 |
| 토 06:00 | `scheduled_control` | BREAK2 ← BREAK1 | 06:00:01.011 | 1.011 | 300 | 10 · 9 · 0 | misfire 0 · ERROR 0 · traceback 0 |
| 토 07:00 | `scheduled_control` | OUT ← BREAK2 | 07:00:01.009 | 1.009 | 122 | 9 · 7 · 0 | misfire 0 · ERROR 0 · traceback 0 |
| 월 06:00 | `scheduled_control` | BREAK2 ← OUT | 06:00:01.006 | 1.006 | 63 | 7 · 9 · 0 | misfire 0 · ERROR 0 · traceback 0 |
| 월 08:00 | `scheduled_control` | IN ← BREAK2 | 08:00:01.013 | 1.013 | 55 | 9 · 11 · 0 | misfire 0 · ERROR 0 · traceback 0 |

- **다섯 경계 모두** 시작·종료 이벤트가 같은 `switch_id` 로 한 쌍이고, 창 안의 전환은 그 한 건뿐이며,
  등록·제거 집합이 전환 전후 `task_*` 집합과 일치한다. `delay_from_boundary_s` 의 1초는 설계상 발화
  시각(매분 1초)이다 — 자세한 수치와 그 값이 무엇의 늦음인지는 아래 항목에 적었다.
- 19:00 에 우리 job 은 IN 트리거를 지우고 BREAK1 트리거(`hour='19-23,0-5'`)로 다시 등록됐다. 06:00 에는 BREAK1 트리거를
  지우고 마무리 트리거(화~토 06:00~06:03 `:14/:44` + 06:04:53)를 등록했고, 마무리 9 슬롯(06:00:14 ~ 06:03:44 여덟 번,
  06:04:53)이 모두 우리 공식 URL 시도와 writer 배너를 남겼다. ⚠️ 배너는 writer **호출**이지 저장 확정이 아니다(값 불변
  회차의 저장 경로는 DEBUG 라 운영 로그에 보이지 않는다 — [SOURCE_HEALTH_PLAN.md §2.2](SOURCE_HEALTH_PLAN.md#22-운영-로그에서-보이지-않는-사실)).
- ⚠️ **다섯 경계 모두 늦지 않은 전환이다.** grace 확대가 필요한 지연 복구는 한 번도 관측하지 못했다 — 복구 경로는
  시험으로만 확인했고, 운영에서의 발생 여부를 일반화하지 않는다.
- 토 07:00 은 우리 마무리 트리거를 **지우기만** 하고 다시 등록하지 않았다 — OUT 에 우리 job 이 없다는 정책과
  일치하며, 전환 뒤 `task_*` job 7개가 OUT 7개다.
- 월 06:00 은 마무리 트리거를 다시 등록했지만 `day_of_week='tue-sat'` 이라 **월요일에는 발화하지 않는다**.
  등록 자체가 정상 동작이고, 9슬롯 실행은 화~토에만 기대한다.
- 월 08:00 은 R1 배포 뒤 새 이미지에서 본 **첫 경계**다(이미지·컨테이너는 위 제목에 적었다).
  R1 은 보고 계측 변경이라 전환 경로를 건드리지 않는다.
- 다섯 경계 모두 `delay_from_boundary_s` ≈ 1.0 초다. 이 1초는 **정책 경계 대비**이고 예정 발화는 매분 1초이므로
  **예정 대비 늦음은 6~13 ms**(래퍼 진입 기준) 다 — 구 grace 1초로도 통과했을 값이고, **grace 30초는 한 번도 필요하지 않았다.**

**정각 misfire 재확인 (2026-09-19 06:1x KST, 읽기 전용)**: 새 컨테이너 StartedAt 은 `2026-09-18T04:26:09.985642228Z` 이고,
고정 창 `docker logs --timestamps --since 2026-09-18T04:26:00Z --until 2026-09-18T21:11:30Z` 의 첫 줄은 `04:26:14Z`, 마지막 줄은
`21:11:29Z`(06:11 KST)다. 이 약 16.7시간(정각 17회)의 출력 176,864 줄(JSON 레코드 173,192)에서 JSON `message` 에 `was missed by` 가 든
레코드와 원문 문자열 `was missed` 는 **둘 다 0건**이다. ⚠️ 첫 줄 시각은 로그 시작만 보여 주며
중간 누락이 없었다는 검증은 아니다. 양성 대조: 구 컨테이너 보존 원문(`stdout.raw` 32개)에서 Docker 타임스탬프 앞머리를 떼고 JSON `message`
의 `was missed by` 를 찾으면 35건이다(추출 창 겹침에 따른 중복이 포함됐을 수 있다 — 앞의 표는 중복 제거 후 19건). 구 컨테이너에서는
정각 12회 중 11회에 misfire 가 있었고 대부분 `report_queue_status` 였다.
⚠️ 사라진 이유는 **모른다.** `report_queue_status` 는 이번 변경과 무관하게 기본 grace 1초 그대로인데도 0건이라 grace 30초의
효과로 설명되지 않는다. 컨테이너 재생성·가동 시간·시간대별 부하 중 무엇 때문인지 가를 근거가 없다.

**정각 정체 조사 — 보류 (2026-09-18 Claude·Codex 합의)**: 원인을 가를 계측 설계(기존 기본 실행기 탐침의 창별 차분 + 루프 지연
탐침)는 만들었으나 구현하지 않았다. 확인한 한계:
- misfire 경고 시각은 **판정·기록 시각**이다(`executors.base.run_job` 이 본문 전에 `datetime.now − run_time > grace` 로 판정) —
  본문만 감싸는 계측은 놓친 실행의 대기를 보지 못한다.
- 하나은행의 정각 첫 시도 로그(예정 `:02`)를 대상으로 한 선택 표본 n=12 에서 시작 지연 p50 0.172초·최대 0.76초였다(직전 예정 초를
  추정했고 9초 초과·예정 후보 없음 표본은 제외). wrapper 진입 뒤 로그라 정체의 시작·끝이나 전체 실행 영향의 상한을 확정하지 않는다.
- broadcast 로그는 실행 조건이 거짓이면 로그 없이 반환하므로 루프 심장박동으로 쓸 수 없다.
- **재개 조건**(하나라도 해당하면 다시 합의): ① 정기 전환이 grace 30초로도 misfire ② misfire 없이 늦게 전환되어 새 모드의 첫
  슬롯을 놓치거나 구 모드 작업이 경계를 넘어 반복 ③ 정각 정체가 다른 사용자 영향(크롤러 타임아웃·broadcast 누락 등)으로 관측
  ④ 사용자가 원인 규명을 우선순위로 지정. 위 **다섯 경계** 관측과 정각 재확인 자료에서 재개 사유는 확인되지 않았다(전체 사용자 영향의
  부재를 입증한 것은 아니다).

**열린 항목 1 판정 (2026-09-21) — 관측 의무의 이행으로 닫는다.**

수정(`086e2ca`) 뒤 경계를 관측하기로 했고, **다섯 경계를 관측했다.** 전부 시작·종료가 같은 `switch_id` 로 한 쌍,
창 안 전환은 그 한 건뿐, misfire 0, 작업 실패 0, 모드별 job 집합 복원 정상이었다. 그 **관측 약속**을 닫는다.

⛔ **"grace 30초가 효과를 냈다" 로 닫는 것이 아니다.** 다섯 경계의 `delay_from_boundary_s` 는 1.006~1.013 초인데
예정 발화가 `CronTrigger(second='1')` 이므로 **예정 대비 6~13 ms** 다. 이 값은 **래퍼 진입 시각** 기준이고
(코드 주석이 명시하듯 `control_job` 의 시작 지연 자체가 아니다) 래퍼 진입은 job 시작 이후이므로 **job 의 늦음은
그 이하**다 — 즉 구 grace 1초(APScheduler 기본값)로도 전부 통과했을 값이다. 그리고 ADR-044 본문이 적어 둔 대로 정각 misfire 가 사라진 원인은 여전히 모른다
(grace 를 바꾸지 않은 `report_queue_status` 도 0건이 됐다). 인과를 이 기록으로 주장하지 않는다.

닫지 않고 남기는 것: **늦은 전환에서 grace 30초가 전환을 살리는 운영 실증이 없다**(시험으로만 확인) /
정각 misfire 소멸의 원인 미상 / 지나간 슬롯 복원·응답→저장 연결 / **열린 항목 2**. 재개 조건 ①~④ 도 그대로다.
ADR-044 전체 완료는 이 기록과 **따로** 판정한다.

### 2026-09-19 후속 — 관리자 토글과 정기 전환 직렬화 (코드 구현, 2026-09-22 `34ef52c` 로 운영 반영)

**결함**: `control_job` 은 `switch_jobs(new)` 가 끝난 **뒤에** `current_mode` 를 바꾼다. 그 사이에 관리자 토글이 들어오면
옛 `current_mode` 로 `switch_jobs` 를 불러 두 본문이 다른 스레드에서 겹친다(정기 전환은 실행기 스레드, 토글은 `async`
라우트 안의 동기 호출이라 이벤트 루프 스레드). 결과는 겹친 순서에 따라 갈린다 — 토글의 옛 모드 등록이 뒤에 남으면
`current_mode` 는 새 모드인데 job 은 옛 모드 트리거로 남아 다음 경계까지 바로잡히지 않고, 같은 job ID 를 양쪽이 등록하면
뒤쪽이 `ConflictingIdError` 로 본문 중간에 끝나 나머지 job 이 빠진다(`replace_existing` 미사용 — APScheduler 3.11.2 에서
확인). 운영에서 이 겹침이 실제로 일어난 적이 있는지는 확인하지 않았다.

**수정**: 모듈 잠금 `_mode_switch_lock`(`threading.RLock`)을 두 호출이 함께 잡는다 — `control_job` 은 모드 판정부터
`switch_jobs`·`current_mode` 갱신까지, `toggle_crawler` 는 `current_mode` 읽기와 `switch_jobs` 를. 토글은 진행 중인 전환이
끝난 뒤 **그때의** `current_mode` 로 재등록한다. 설정 캐시 갱신은 잠금 밖에서 먼저 끝나므로 마지막 재등록은 새 설정을 쓴다.
- 전환 기록의 `delay_from_boundary_s` 에는 이제 토글을 기다린 시간이 들어간다(래퍼 진입이 잠금 안이다).
- 위 기록 표의 "겹친 다른 호출" 주의는 이 두 경로 사이에서는 더는 해당하지 않는다. `switch_jobs` 를 부르는 다른 경로가
  생기면 다시 해당한다.

**비용과 교착**: 토글이 전환과 겹치면 그 전환이 끝날 때까지 이벤트 루프가 멈춘다. 경계 관측의 전환 호출 시간(`duration_ms`)은
61·300 이었고 상한을 보장하지는 않는다. 토글 자신의 `switch_jobs` 도 원래 루프 위에서 동기로 돌았다. 잠금 순서는 항상 `_mode_switch_lock`
→ APScheduler 내부 잠금이고, 전환 본문의 `add_job`·`remove_job` 은 루프를 기다리지 않는다(루프 깨우기는
`call_soon_threadsafe` 예약뿐이고, 등록된 리스너는 max_instances·missed 이벤트만 받는다).

**시험**: `tests/test_control_job_transition.py` 의 `TestToggleSerialization` — 정기 전환 본문을 게이트로 붙잡은 사이 다른
스레드에서 토글하면, 토글 본문은 전환이 끝난 뒤 시작하고 새 모드로 재등록한다. 수정 전 코드에서는 이 시험이 실패했다(토글
본문이 진행 중인 정기 전환 안에서 시작했다). 잠금을 푼 **뒤에** `current_mode` 를 갱신하는 변이는 스레드 경합 시험으로는
살아남아서(3회 모두), 잠금 해제 순간의 `current_mode` 를 기록하는 시험을 따로 두었다. 잠금을 기다리는 사이 경계를 넘으면
기다린 뒤의 시각으로 모드를 정하는지도 따로 본다 — 대기 전 시각으로 정하면 옛 모드에 머물고, 경계 직후의 정기 실행은
`max_instances=1` 로 건너뛰어져 다음 분까지 전환이 늦는다. 변이 7종(토글·정기 전환 각각 잠금 제거, 토글이 잠금 전에 모드 읽기,
잠금 뒤 모드 갱신, 모드 없음 가드 제거, 두 경로가 서로 다른 잠금, 잠금 전 모드 판정)은 각 3회 모두 잡혔다.

**운영**: 2026-09-22 배포 `34ef52c` 에 포함됐다(switch 06:17, promote 08:37 KST — DEPLOYMENT §8.1b). 같은 날 08:00 BREAK2→IN 전환 한 쌍이
정상(경계 뒤 1.0초, 실패 0)이었다. 열린 항목 2 는 그대로 열려 있다.

## ADR-043: OUT(주말) 수집 주기 1분 상향 — 판정 기준과 롤백값을 함께 고정

**날짜**: 2026-08-28
**상태**: Accepted — `ab36ad0`/`540c32f` push 및 2026-08-28 운영 배포. 2026-09-26 4주말 재측정·권고 기록(아래 "판정 기록") — **실험 종료는 사용자 GO 전까지 열림**
...
### 판정 기록 (2026-09-26 — 권고, 종료 전)

**재측정**: 변경 뒤 4주말(08-29·09-05·09-12·09-19, 각 토 07:00~월 06:00 KST) OUT 구간 `bank_exchange_rates`·`investing_exchange_rates`
변경행(운영 RDS 읽기 전용). 은행 표는 30일 보관이라 변경 전 은행 자료는 위 "맥락" 표(변경행)를 그대로 쓴다.

| 소스 | 변경 전(위 표, 구 주기) | 변경 뒤 4주말(1분) |
|---|---|---|
| hana | 18행 | 33행 |
| shinhan | 6행 | 9행 |
| nh | 0 | 3행 |
...
⚠️ investing 변경 전 값: 이번 재조회(장기 보관 표)에서 08-01 주말 3행(3통화 1사건)으로 나와 위 표의 "1"과 단위가 다르다 — 원표는 고치지 않고 차이만 적는다.

**해석**(change-only 저장이라 "0"은 고시 없음이 아니라 "그 해상도에서 추가 변경을 관측하지 못함"):
- hana: 09-05 15:49:38 → 15:50:38 두 변경이 구 슬롯(매 10분 `:45`) 한 칸 안에 있어 **구 주기에선 중간값을 놓쳤을 사례**가 관측됐다.
  (09-20 11:18→11:23 은 사이에 구 슬롯 11:20:45 가 있어 근거가 아니다.)
- shinhan: 3건 중 2건(09-05·09-12 토 07:00:3x)은 BREAK1 수집 종료 뒤 **첫 OUT 실행**의 포착 — 구 설정의 다음 예약은 07:13:45(큐 제출 시각)이며, 값이 그때까지 유지됐다면 포착됐을 것이다.
  주말 중간 변경은 09-19 19:22:32 한 건.
- nh: 09-12 17:40:11 한 건. kb·bs·investing: 네 주말 모두 1분 수집의 추가 수확을 확인하지 못했다(investing 은 주말 휴장이 보조 설명).

**비용 축**(위 판정 기준, 운영 컨테이너 cgroup v2 읽기 전용 체크포인트 — 원문 `design/adr043/cost_cp_*.txt`):

| 창(KST, 2026-09-26 토) | 모드 | Δnr_throttled/Δnr_periods | CPU 사용 | memory.events max 증가 · oom_kill | Chrome 전체·60초 초과(시작→끝) |
|---|---|---|---|---|---|
| 07:06→08:06 | OUT(현행 1분) | 9.8% (2,792/28,390) | 381 s | 0 · 0 | 0·0 → 0·0 |
| 04:05→05:06 | BREAK1(비교용) | 12.3% (3,709/30,099) | 607 s | 0 · 0 | 0·0 → 0·0 |

- 두 창은 모드가 달라 대조군이 아니다. 측정한 창에서 OUT의 CPU 사용시간과 throttling 발생 비율은 BREAK1보다 낮았다. 두 창 모두 memory.events 전 항목 증가분은 0이고,
  시작·끝 체크포인트의 Chrome 전체·60초 초과 수는 0이었다. 창 내부의 일시적 프로세스 증가나 고아 여부는 확인하지 않았다.
- 구 설정 OUT 의 비용은 측정하지 않았으므로 **1분 전환의 순증분은 모른다**. 과거 네 주말의 비용도 소급 측정하지 못했다(한계).
- 따라서 아래 복귀 권고의 근거는 비용 측정이 아니라 판정 기준의 **수확** 축이다(이번 체크포인트는 구 OUT 주기 대비 비용 증분이나 소스별 비용을 분리하지 못하므로, 복귀 여부의 독립 근거로 쓰지 않는다).

**권고**(Claude·Codex 합의): hana **1분 유지**. kb·bs·investing·nh·shinhan 은 위 "롤백값" 표의 trigger·grace 로 **복귀**
(nh·shinhan 은 subprocess 비용 대비 4주말 각 1건 수확 — 복귀는 지연과 중간값 누락 가능성을 감수하는 선택). DXY 는 대상 아님.
**적용은 별도 코드 변경·배포**(09-29 배포 대상 `b0b44f1` 에는 넣지 않는다). **실험 종료·롤백 이미지 `rollback-2026-08-28` 의 이 ADR 쪽 묶임 해제는
사용자 GO 와 종료 시각 기록 뒤**(종료 기록에는 실행 분모·비용 지표·환경 식별자를 남긴다).
원문: 로컬 설계 폴더 `design/adr043/`(측정 스크립트·출력).
