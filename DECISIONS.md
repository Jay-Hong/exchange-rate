...
- server 기준 commit: `0fd18c8a3ff82a2988dee6b34903e1b42301d2c8`
- iOS 기준 commit: `8f6afff299621d50c3431dbea739ed07c378c59a`
- archive SHA: `cde1d2ca3e714733776e1b0d7e821a542e1f8d183cb2951bef8c93fb444d9814`
- manifest SHA: `231fabd5d652d09f460095bfa130772deb7592f06bb2303487d5f222c63bc248`
...
- publisher 모듈 자체에는 timer 가 없다(baseline B1 의 **범위 한정**). 외부의
  ...
  (`app/scheduler.py:1907-1913` · `app/main.py:980-1003`). 따라서 현재 경로에는
...
**경계 전환 관측 (2026-09-18·19, 컨테이너 `97abe1128e46…`·이미지 `683de70a…`, 재시작 0)**:
관측 스크립트가 경계 뒤 3~7분에 서버에서 Docker 로그를 읽어 `mode_switch_*` 이벤트를 JSON 으로 다시 파싱했다
(검사기 SHA-256 `4bf19484…`, 결과 원문은 세션 작업 폴더의 `observe_fri1900.txt`·`observe_sat0600.txt`).
...
- 두 경계 모두 시작·종료 이벤트가 같은 `switch_id` 로 한 쌍이고, 창 안의 전환은 그 한 건뿐이다. `delay_from_boundary_s`
  의 1초는 설계상 발화 시각(매분 1초)이다 — 예정 발화 대비 늦음은 약 10 ms 다.
...
- ⚠️ 이 두 번은 **늦지 않은 전환**이다. 이번 두 경계에서는 grace 확대가 필요한 지연 복구를 관측하지 못했다 — 복구 경로는
  시험으로 확인했고, 운영에서의 발생 여부를 일반화하지 않는다. 나머지 경계(08:00·토 07:00·월 06:00)는 관측하지 않았다.
...
- **재개 조건**(하나라도 해당하면 다시 합의): ① 정기 전환이 grace 30초로도 misfire ② misfire 없이 늦게 전환되어 새 모드의 첫
  ...
  ④ 사용자가 원인 규명을 우선순위로 지정. 위 두 경계 관측과 정각 재확인 자료에서 재개 사유는 확인되지 않았다(전체 사용자 영향의
  부재를 입증한 것은 아니다).

⛔ 열린 항목 1 의 종결 여부와 ADR-044 전체 완료는 이 기록과 **따로** 판정한다(열린 항목 2 는 그대로 열려 있다).
