...
#
# ⚠️ 현재 D > T 는 **단순한 튜닝 선택**이다 — "분류가 보존된다"는 보장이 아니다.
#    ⛔ 한때 "D > T 면 SDK 가 먼저 끝나 오류 분류가 클라에 도달한다"고 적었는데 **과장이다**
#       (codex Medium): T 는 **per-attempt** 상한이고(SDK 가 requests 에 `timeout=` 으로 넘긴다),
#       한 번의 검증은 **인증서 조회 + accounts:lookup × (1 + status 재시도 4회) + backoff** 를
