...
반환했다.
**저장 데이터** 피해 범위: 일봉 close write는 차단됐다 — close write gate가
contract identity에서 REJECT. 그러나 **raw tick 466건과 hourly 4버킷에는 잘못된
월물(A75608)이 기록**됐다(정책상 07:00부터 차월물이어야 함). 두 store 모두
retention(각 30일·14일)으로 소멸하고 A75609 intraday를 되살릴 소스가 없어 정정
불가다. "오염 write 0"은 일봉 축에 한정한 서술이다.

**소비 측 피해**(저장 데이터와 별개 축): 그날 KRX 가격알림 4건이 활성이었고
(`source_notification_settings`), 06:00·10:33·10:45·10:55에 FCM이 실제 발사됐다.
그 평가는 **차월물이어야 할 시점에 A75608 가격으로** 이뤄졌고, 11:30 이후로는
tick이 끊겨 **평가 자체가 정지**했다(발화했어야 할 건이 있었는지 확인 불가).
두 영향 모두 소급 정량화가 불가능하다.
