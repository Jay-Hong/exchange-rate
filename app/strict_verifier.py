...
# **검증 단계 전체**의 시간 상한. 호출별 상한이 아니다.
# ⛔ 구 문구는 "§D6의 ack timeout 10초 아래"라고 적어 클라 데드라인 안에 든다는 인상을 줬는데
#    **틀렸다**: 이 8초는 검증만이고 설계상 뒤에 상한 없는 lock 대기와 ack 송신(5s)이 더 붙는다.
#    알려진 몫만 더해도 8+5=13s로 이미 10s를 넘는다. 요청→ack 상한은 아직 계약이 아니다(§D-const).
#
