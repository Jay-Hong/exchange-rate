...
연결**의 만료 통지가 D-const의 "만료→통지" 관측 계약을 넘긴다.
⚠️ 이때 §B1이 지키는 것은 **인가 판정의 상한**이지 배달의 상한이 아니다 — 이미 판정을 통과해
in-flight인 전송에는 §B5(e)의 열린 창이 그대로 적용된다. 경합하면 그 연결만 건너뛰고 다음 주기로 미룬다.
...
⚠️ 관측 지연 식의 **정본은 계획 D-const**다 — `(연속 skip + 1) × 주기 + (lock + notify + close)`
이고 **유한 상한이 아니다**(연속 skip에 상한이 없다). 여기서 그 값을 다시 정의하지 않는다.
...
# 경합 시 **대기** 상한. 짧아야 한다 — 이 값은 **경합 중이라 건너뛰는** 연결이 사이클을 붙드는
# 상한이다. ⚠️ 통지·close까지 진행하는 연결의 상한은 `lock + notify + close`다(위 docstring) —
# 한때 이 주석이 "이 값이 곧 한 연결이 사이클을 붙드는 상한"이라고 적어 그 문장과 **모순**이었다.
# 이 값의 역할은 사이클이 **연결 수 K에 비례하지 않게** 막는 것이다.
...
async def sweep_once(
    ...
    *,
    ...
    #    0.000/0.051/0.102/0.154초로 누적) 기본값에서는 정지 연결 3개만으로 뒤 연결의 통지가
    #    15초 밀린다 — D-const 관측 지연 식의 `(lock + notify + close)` 항은 **그 연결 자신의
    #    몫**이어야 하는데 직렬이면 K배가 된다. (D-const에 "만료→통지 10초" 상한은 **없다** —
    #    지키는 것은 상한이 아니라 격리다.) 각 작업은 **서로 다른 연결 lock**을 잡으므로 §B4의
    #    직렬화 계약(연결 단위)은 그대로다.
