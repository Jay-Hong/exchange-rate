...
def crawl_and_save_citi_bank_exchange_rates():
    ...
    report = bank_report.start_report(
        logger, BANK_NAME, tuple(PAIRS),
        (bank_report.OFFICIAL_PRIMARY, bank_report.OFFICIAL_SECONDARY, bank_report.MIBANK))
    try:
        _crawl_and_save_citi(report)
...
def _crawl_and_save_citi(report):
    primary = bank_report.PathObserver(report, bank_report.OFFICIAL_PRIMARY)
    secondary = bank_report.PathObserver(report, bank_report.OFFICIAL_SECONDARY)
    mibank = bank_report.PathObserver(report, bank_report.MIBANK)
    db = SessionLocal()
    try:
        logger.info(f"CITI_BANK_URL 시도", extra={"bank": BANK_NAME})
        primary.start()
        crawl_and_save_citi_first_routine(CITI_BANK_URL, CITI_BANK_SELECTORS, db, observer=primary)
        primary.finish()
    except Exception as e:
        primary.finish(error=e)
        logger.exception("CITI_BANK_URL 크롤링 실패", extra={"url": CITI_BANK_URL})
        
        try:
            logger.info(f"SECOND_CITI_BANK_URL 시도", extra={"bank": BANK_NAME})
            secondary.start()
            crawl_and_save_routine(SECOND_CITI_BANK_URL, SECOND_CITI_BANK_SELECTORS, db, observer=secondary)
            secondary.finish()
        except Exception as e:
            secondary.finish(error=e)
            ...
            # 2차 경로: 취소 등은 기존대로 전파한다 — 종료 원인만 결속한다.
            secondary.finish(error=interrupted)
            raise
    except BaseException as interrupted:
        # 1차 경로: 취소 등은 기존대로 폴백 없이 전파한다(`except Exception` 을 넓히지 않는다).
        primary.finish(error=interrupted)
        raise
    finally:
        db.close()
