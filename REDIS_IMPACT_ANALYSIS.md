...
- investing: 6회 (10초마다)
- dxy_spot: 6회 (10초마다)
- kb, hana: 각 6회 (10초마다, 2026-09-01 IN-only canary)
- woori: 2회 (30초마다, 2026-09-01 IN-only canary)
- bs, citi: 각 1회 (60초마다)
- shinhan, ibk, nh, sc: 각 1회 (60초마다)
총: 6×4 + 2 + 1×6 = 32회/분

평균 크롤러 시작률: 32/60 = 약 0.53 runs/s

DB 비교 SELECT는 크롤러 실행당 1회로 고정되지 않는다. 은행·Investing writer는
추출된 통화별로 마지막 행을 조회하고, DXY는 저장 경로와 direct-latest flag에 따라
추가 조회가 생길 수 있다. 따라서 0.53은 **DB qps가 아니라 job 시작률**이며 실제
DB qps와 쿼리 시간은 RDS 프로파일링으로 측정해야 한다.
...
**Redis 조회로 대체:**
```
환율 비교 SELECT를 Redis HGET으로 대체한다고 가정
...
| RDS 쿼리 빈도 | 미측정 (크롤러 시작 0.53 runs/s) | 변경 감지 시 중심 (추정) | DB 읽기 대부분 제거 가능 |
...
피크 시간 (08:30-21:00): 추정 불가 (부하 테스트 필요)
  - 크롤러 부하: 시작률 0.53 runs/s, DB qps 미측정
...
| 크롤러 시작률 | 0.53 runs/s | 스케줄 기반 계산 (DB qps는 미측정) |
...
|---------|-----------|-------|------|
| **크롤러 환율 비교** | DB qps 미측정 (시작률 0.53 runs/s) | 미미 | 효과 제한적 |
...
- ❌ 크롤러 환율 비교는 **효과 제한적** (시작률 0.53 runs/s, DB qps는 실측 필요)
...
| | 크롤러 환율 비교 | ❌ 보류 | - | 효과 미미 추정 (시작률 0.53 runs/s) |
| **Phase 2 (PostgreSQL)** | | | | |
...
2. **주요 부하 파악**: 크롤러 시작률은 0.53 runs/s이며 DB qps는 실측 필요;
   현재 추정상 WebSocket/Broadcasting이 주요
