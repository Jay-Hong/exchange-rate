...
> ⚠️ **Bithumb REST `trade_timestamp` +9h 비대칭 (2026-06-22 실측, hotfix)**: ③ super-lite의 "`timestamp_ms` = clean UTC epoch (5 source 전수 확인)" 전제는 **Bithumb REST에서만 거짓이었다**. Bithumb **REST** `v1/ticker`의 `trade_timestamp`/`timestamp`는 KST 벽시계를 UTC epoch처럼 인코딩해 true UTC보다 **+9h(=32400s) 미래** (라이브 3회 교차 +8.998~9.0h). 같은 v1 스키마인데 **WS는 정상**(diff≈0). REST fallback probe가 이 +9h를 `seen_at`(Redis)·`event_ms_to_utc_naive`(DB)로 흘리면 regression guard([latest_rates_cache.py:492](app/latest_rates_cache.py#L492))가 이후 정상 tick을 **~9h freeze**시킨다 — 사용자가 본 "미래 시각 고정" 증상. **fix**: REST 전용 `_bithumb_rest_event_ms`([usdt_sources.py](app/crawlers/usdt_sources.py))가 `trade_date_kst`/`trade_time_kst`(없으면 UTC 필드)로 epoch 재구성 + raw 폴백에 future-skew fail-closed 가드. WS parser는 무변경(정상). upbit/coinone/korbit/gopax REST는 **동시 1회 교차측정 기준** 정상(≈0h, 지속 재측정 미수행이라 확정 단정 아님) — 본 fix는 Bithumb REST 전용이라 4종 상태와 무관. **배포 후 운영 — freeze 복구 절차 (2026-06-22 실측 정정, 구 'deploy→DEL' 순서 오류)**: Redis DEL **만으론 부족** — running fastapi의 in-memory `_last_written_usdt_state`가 오염 `seen_at`을 warm 캐시하고, force-recreate해도 fresh process 첫 tick이 **아직 안 지운 Redis에서 cold-start re-read로 +9h를 재캐시**한다(set_latest_usdt cold-start 분기). **정확한 순서**: ① (오염 원천이 코드 버그면) 코드 fix 배포 = build+force-recreate(재오염 차단) → ② `DEL latest:source:bithumb:usdt-krw` → ③ FastAPI **force-recreate**(빈 Redis에서 clean cold-start = in-memory state 초기화) → ④ 정상 tick 후 Redis `seen_at`이 현재 시각으로 복원됐는지 확인. ⚠️ **DEL이 freeze-clear용 force-recreate보다 먼저여야 함** — 'recreate→DEL' 순서는 recreate가 미삭제 Redis를 재캐시해 freeze 지속(2026-06-22 실측: DEL 후 MISSING 지속 → 추가 force-recreate로 해소). DB는 insert-if-changed라 +9h probe 시 rate 미변경이면 future row 미생성 가능(이번 사고 DB future row 0건 = Redis-only freeze, DB cleanup 불요였음).
>
> ✅ **super-lite first PR land** (2026-06-11, 세 리뷰어 수렴): 아래 설계(신규 `exchange_ts` 컬럼 + migration)는 **불필요로 판명**. 기존 `insert_source_rate_if_changed(timestamp=)` param + `timestamp DESC` 쿼리가 이미 받을 수 있어, USDT 5 WS DB writer가 tick의 exchange event ts(`crud.event_ms_to_utc_naive`)를 그 param으로 전달 → **out-of-order stale이 latest로 오판되지 않음 (migration/watermark/read-path 0)**. DB writer 경유 전체(WS tick + REST probe fanout) 적용, 직접 crud 호출(KRX/legacy)만 미적용. 핵심 근거 = **DB는 history store**라 stale도 제 ts로 보존(Redis latest-only는 skip이 맞지만 DB는 다름). Redis 가드(seen_at)와 동일 `timestamp_ms` 사용이라 시맨틱 일관(5 source 전수 확인). full(영속 컬럼)은 추가 가치(재시작 직후 첫 write 가드)가 plumbing 비용 대비 약해 **보류·폐기**. **KRX tick ts — 스코핑 완료, 별 PR defer** (2026-06-12, 세 리뷰어 수렴): KRX WS tick에도 super-lite 적용 가능하나 **USDT helper 그대로 재사용 아님 + 가치 작음** → defer. **스코핑 사실 4**: (1) KIS frame `bsop_hour`=HHMMSS 체결시각, **날짜 필드 없음**([kis_futures.py](app/sources/kis_futures.py)) → 날짜 별도 결합 필요. (2) **기존 ts 혼재** — WS `KrxDbWriter`([krx_kis.py:1754](app/crawlers/krx_kis.py#L1754))는 timestamp 미전달→now()(save-time) / close snapshot([2865](app/crawlers/krx_kis.py#L2865))은 `boundary_utc_naive`(event-time) → WS=save·close=event 현행 혼재(WS exchange-ts는 신규 시맨틱 아닌 **정규화 방향**). (3) **소비자** [`usdt_topic_payload.py:333`](app/usdt_topic_payload.py#L333) `get_latest_source_rate(db,"krx",...)` = **Redis-miss DB fallback 전용**(노출 창 좁음). (4) **midnight crux** — `bsop_hour`(no-date)를 단순 `received_at.date` 결합 시 23:59:59 체결이 00:00 이후 도착하면 ~24h 미래 오결합(CM 야간 17:50~06:00이 자정 통과). ⚠️ **naive-join 선례 재사용 금지**: close writer [`_parse_event_at_kst`(2202)](app/crawlers/krx_kis.py#L2202)가 `received_at.date + bsop_hour` 결합 선례이나 **CF 15:45/CM 06:00 grace window(자정 무관)에서만 안전** — WS 일반 경로(자정 통과)에 무비판 재사용 시 +24h 버그. **미래 PR 설계 방향**: midnight-aware date 결합 helper — D-1/D/D+1 후보 중 **received_at 이전 우선 + 작은 future-skew 허용 + 최대 지연 상한 초과/malformed → received_at fallback**, WS+close writer 공유. **defer 근거**: fallback 전용 소비자 + 단일 ordered WS(1초 debounce, 발생원 reconnect/세션경계/close-write 교차 한정) + rollup 영향 미미 → 가치 작음 vs helper 비용(자정 테스트·rollup 회귀) 비대칭. **재개 trigger(하나라도)**: (i) KRX DB fallback 사용 빈도↑(Redis 불안정 신호) / (ii) KRX stale-latest 실사건 관측 / (iii) 6/15 후 gate-checked write 활성화 이후 close-write↔WS tick latest 교차 이상. 아래는 **super-lite 전 원 설계 기록**(역사 보존):
