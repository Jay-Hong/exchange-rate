...
| **woori** | 평일 08:30 | **05:55:56 관측(2026-09-05), 종료 시각 미확정** | ✅ (~05:59:44) | ⚠️ 마무리 조회만 (06:00~06:03 `:14/:44` + 06:04:53, 화~토) | ❌ |
...
- 정적 HTML 파싱
- **부산·씨티 결과 보고(R1a·R1b, 보고 전용·미배포)**: `bank_report.py` 가 회차마다 `bank_round_started`/`bank_round_finished`
  를 기존 은행 로거로 남긴다. 공식 루틴과 공유 `crawl_mibank_rates` 는 `observer=None` 인자로 실제 추출에 쓴 요소·값·
  판별 근거만 넘긴다(다른 7개 은행은 관측자를 넘기지 않아 기록 없음). 범위 검사·편차·채택·writer 호출은 호출
  지점에서 기록한다. 폴백 순서·총실패를 삼키는 기존 동작·MIBANK 반환/예외·writer 입력은 그대로다.
...
- 2026-09-10: **수집 창을 한 시간 옮겼다.** BREAK1 `19:00~05:59` `:14/:44`(1,320회) +
  BREAK2 마무리 조회 `06:00~06:03 :14/:44` + `06:04:53`(화~토, 9회). 근거는 사용자가 은행
  페이지에서 확인한 **2026-09-05(토) 05:55:56 고시**다. 그 시각은 구 창(05:04:53 종료)의
  51분 뒤였고, 토요일이라 이후 월요일 08:00까지 **예약된 수집 기회가 없었다**(BREAK1 은
  05:04:53 이후 발화 없음, BREAK2·OUT 은 미등록). 운영 DB 는 09-05 03:55:44 다음 행이
  09-07 08:00:14 다 — ⚠️ 이 기록만으로 놓친 고시의 횟수·동일성은 확정할 수 없다.
  ⚠️ 05:55:56은 "05:00 이후에도 고시한다"는 **관측**이지 "정확히 06:00에 끝난다"는 증거가
  아니다. 06:00은 우리가 정한 **수집 종료 정책**이다.
  ⚠️ **토 06:04:53 ~ 월 08:00은 미관측 구간**이며, 그 구간에 추가 고시가 없음을 보장하지
  않는다. OUT 주말 수집은 이번 범위에 넣지 않았다.
  ⛔ 마무리 조회는 **같은 `task_woori` ID**를 쓴다(다른 ID면 executor 카운터가 갈려 동시
  실행). 이는 크롤러 **호출** 중복 방지이지, 타임아웃 뒤 남는 Chrome 문제는 별개다.
